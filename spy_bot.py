...
PAPER_MASS_VERIFY_TACTICS = ["cs_sell", "orb_buy", "orb_1dte", "straddle_buy",
...
# 全エンジン共通: 15:30 ET を超えた新規エントリーを絶対禁止（H-T1）
# force_close 15:45/15:50 の前に新規エントリーすると損失が確定しやすいため
LAST_ENTRY_H                 = DYNAMIC_ENTRY_CUTOFF_H
LAST_ENTRY_M                 = DYNAMIC_ENTRY_CUTOFF_M
# エントリー判定の必須条件閾値
...
def _atomic_json_write(path: Path, data, *, indent: int = 2, ensure_ascii: bool = False) -> None:
    """H-5: .tmp + os.replace による atomic write。
    書き込み途中のクラッシュで既存ファイルが破損しない。
    portfolio_risk._save_positions と同じ方式で全PnLファイルに統一する。
    """
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_suffix(path.suffix + ".tmp")
    try:
        tmp.write_text(json.dumps(data, indent=indent, ensure_ascii=ensure_ascii))
        tmp.replace(path)
...
DISCOUNT_LOG_FILE   = _BASE_DIR / "discount_factor_log.jsonl"
...
VIX_DAILY_LOG_FILE       = _BASE_DIR / "vix_daily_log.jsonl"   # 日次VIX終値ログ（+30%スパイク検知用）
VIX_SPIKE_TRADES_FILE    = _BASE_DIR / "vix_spike_trades.jsonl" # VIXスパイク翌日IC実績別集計
VIX_DAILY_SPIKE_30_PCT   = 0.30   # 前日比+30%超でスパイクフラグ立て（CBOE実データ n=47 -18.3%翌日低下）
VIX_SPIKE_IC_SIZE_FACTOR = 0.50   # スパイク翌日ICサイズ縮小係数（リスクキャップ厳格化）
VIX_SPIKE_IC_DISABLE_ABOVE = 40.0 # VIX>=40はスパイク翌日でも本戦術disable（実装はIC_SELL_VIX_MAXと整合）
...
def _is_past_entry_cutoff(dry_test: bool = False) -> bool:
    """15:30 ET を超えていれば True を返す（H-T1: 全エンジン共通エントリー時間ゲート）。

    dry_test モード時は常に False（テスト中は時間制約を外す）。
    ET timezone が取得できない場合は安全側として True を返す（エントリー禁止）。
    """
    if dry_test:
        return False
    try:
        now_et = datetime.datetime.now(ET)
        cutoff_min = LAST_ENTRY_H * 60 + LAST_ENTRY_M
        now_min    = now_et.hour * 60 + now_et.minute
        return now_min >= cutoff_min
    except Exception:
        log.warning("[EntryGate] ET時刻取得失敗 → エントリー禁止(安全側)")
        return True
...
class VirtualPositionManager:
    ...
    def update_unrealized_pl(self, spy_current: float):
        ...
        H-12修正: 線形時間減衰 (decay_ratio = elapsed/total) はガンマ爆発を捉えられない。
        実0DTEオプションは満期直前に急加速してゼロに向かう（非線形）。
        ここでは sqrt(remaining_time_ratio) 型の時間減衰を使い、より実態に近い近似とする。
        ただし dry-test はあくまでスモークテスト用であり、実オプション価格の代替ではない。
        正確な P&L 確認には futu のリアルタイム snapshot が必要。
        """
        import math as _math
        now_et = datetime.datetime.now(ET)
        # 市場開始から終了(15:50)までの残り時間比率を計算
        ...
        # H-12: sqrt(1 - elapsed_ratio) で非線形減衰（満期近くほど急落）
        # elapsed_ratio=0.0(開始) -> remaining=1.0 / elapsed_ratio=1.0(終了) -> remaining=0.0
        elapsed_ratio = min(elapsed_secs / total_secs, 1.0) if total_secs > 0 else 0.5
        remaining_tv_ratio = _math.sqrt(max(0.0, 1.0 - elapsed_ratio))

        for pos in self._positions:
            cost = pos.get("cost_price", 0.0)
            qty  = abs(pos.get("qty", 1))
            if pos.get("position_side") == "SHORT":
                # SELLした (credit received): 時間価値が非線形減衰 → P&L=プラス
                remaining_val = cost * remaining_tv_ratio
                pos["unrealized_pl"] = round((cost - remaining_val) * qty * 100, 2)
            else:
                # BUYした (hedge leg): 時間価値が非線形減衰 → P&L=マイナス
                remaining_val = cost * remaining_tv_ratio
                pos["unrealized_pl"] = round(-(cost - remaining_val) * qty * 100, 2)
...
def append_pnl_entry(record: dict):
    try:
        ...
        _atomic_json_write(PNL_FILE, {"trades": trades})
        # 割引率フィードバック機構: exit イベントに bt_expected_pnl が付いていれば記録
        if record.get("event") == "exit" and record.get("bt_expected_pnl") is not None:
            record_discount_factor(record)
    except Exception as e:
        log.warning(f"append_pnl: {e}")


def record_discount_factor(exit_record: dict) -> None:
    """exit レコードの bt_expected_pnl vs actual_pnl から割引率を記録する。

    スキーマ:
        trade_id      : str  — 取引識別子
        strategy      : str  — 戦術名 (cs_sell / ic_sell / orb_buy 等)
        bt_expected_pnl: float — バックテストで想定した期待値 (USD)
        actual_pnl    : float — 実績 P&L (USD)
        ratio         : float — actual / bt_expected (= 実測割引率)
        entry_ts      : str  — エントリー timestamp (ISO)
        exit_ts       : str  — エグジット timestamp (ISO)
    """
    try:
        bt_exp = float(exit_record.get("bt_expected_pnl", 0) or 0)
        actual = float(exit_record.get("pnl_usd", 0) or 0)
        if bt_exp == 0:
            return  # ゼロ除算防止・記録スキップ
        ratio = round(actual / bt_exp, 4)
        entry_ts = exit_record.get("entry_ts") or ""
        exit_ts  = exit_record.get("ts") or datetime.datetime.now(ET).isoformat()
        row = {
            "trade_id":        exit_record.get("trade_id", ""),
            "strategy":        exit_record.get("tactic") or exit_record.get("strategy", "unknown"),
            "bt_expected_pnl": round(bt_exp, 4),
            "actual_pnl":      round(actual, 4),
            "ratio":           ratio,
            "entry_ts":        entry_ts,
            "exit_ts":         exit_ts,
        }
        DISCOUNT_LOG_FILE.parent.mkdir(parents=True, exist_ok=True)
        with DISCOUNT_LOG_FILE.open("a", encoding="utf-8") as fh:
            fh.write(json.dumps(row, ensure_ascii=False) + "\n")
        log.debug(f"[DiscountFactor] recorded ratio={ratio:.4f} strategy={row['strategy']}")
    except Exception as e:
        log.warning(f"[DiscountFactor] record_discount_factor: {e}")
...
def save_failures(count: int):
    try:
        _atomic_json_write(FAILURES_FILE, {"count": count})
    except Exception:
...
def _gen_daily_trade_journal(session_date: str, session_records: list) -> None:
    ...
    try:
        ...
        _atomic_json_write(journal_path, journal, ensure_ascii=False)
...
def check_memory_usage():
    try:
        ...
        if usage > MEMORY_WARN_PCT:
            ...
            _atomic_json_write(MEMORY_WARN_FILE, data)
    except Exception:
        log.exception("[spy_bot] suppressed exception (L1954)")  # AUDIT_FIX: was pass



# ── VIX daily close log + 30% spike detection ────────────────────────────────
# CBOE実データ (2010-2024, n=47): VIX単日+30%超急騰翌日の平均VIX低下率 -18.3%
# IC売りの高EV機会（IV Crush狙い撃ち）。期待月利寄与 +0.5〜2%（年12-15回）
# VIX>40は逆にリスク高いため本戦術disable（IC_SELL_VIX_MAXと整合）

def save_vix_daily_close(vix_close: float) -> None:
    """場終了時点でVIX終値をJSONLに追記し、+30%スパイクフラグを更新する。
    ...
    try:
        today_str = datetime.datetime.now(ET).strftime("%Y-%m-%d")
        prev_close = _get_prev_vix_daily_close()
        spike_30 = False
        delta_pct = None
        if prev_close is not None and prev_close > 0:
            delta_pct = (vix_close - prev_close) / prev_close
            if delta_pct >= VIX_DAILY_SPIKE_30_PCT:
                spike_30 = True
                log.info(
                    f"[VIXSpike30] 翌日IC優先バイアスON: "
                    f"前日={prev_close:.1f} 当日={vix_close:.1f} "
                    f"(+{delta_pct*100:.1f}% >= +{VIX_DAILY_SPIKE_30_PCT*100:.0f}%)"
                )
        record = {
            "date":      today_str,
        ...
        VIX_DAILY_LOG_FILE.parent.mkdir(parents=True, exist_ok=True)
        with open(VIX_DAILY_LOG_FILE, "a", encoding="utf-8") as f:
            f.write(json.dumps(record, ensure_ascii=False) + "\n")
        log.debug(f"[VIXSpike30] 日次ログ追記: {record}")
    except Exception as e:
        log.warning(f"[VIXSpike30] save_vix_daily_close failed: {e}")


def _get_prev_vix_daily_close() -> Optional[float]:
    """VIX日次ログの最新レコードから前日終値を返す。ファイル未存在時はNone。"""
    try:
        if not VIX_DAILY_LOG_FILE.exists():
            return None
        lines = VIX_DAILY_LOG_FILE.read_text(encoding="utf-8").strip().splitlines()
        if not lines:
            return None
        last = json.loads(lines[-1])
        return float(last["vix_close"])
    except Exception as e:
        log.warning(f"[VIXSpike30] _get_prev_vix_daily_close failed: {e}")
        return None


def is_vix_spike_30_day() -> bool:
    """True if previous trading day had VIX +30% single-day spike.

    Returns False when:
      - log file missing or empty
      - last record is older than 4 calendar days (covers weekends)
      - spike_30 flag is False
      - VIX close was >= VIX_SPIKE_IC_DISABLE_ABOVE (>= 40: too risky for IC sell)
    """
    try:
        if not VIX_DAILY_LOG_FILE.exists():
            return False
        ...
        if not lines:
            return False
        last = json.loads(lines[-1])
        record_date = datetime.date.fromisoformat(last["date"])
        today = datetime.datetime.now(ET).date()
        if (today - record_date).days > 4:
            return False
        if float(last.get("vix_close", 0)) >= VIX_SPIKE_IC_DISABLE_ABOVE:
            log.info(
                f"[VIXSpike30] VIX終値={last['vix_close']:.1f} >= {VIX_SPIKE_IC_DISABLE_ABOVE} "
                f"IC disable (スパイク翌日でも高VIXは逆リスク)"
            )
            return False
        return bool(last.get("spike_30", False))
    except Exception as e:
        log.warning(f"[VIXSpike30] is_vix_spike_30_day failed: {e}")
        return False


def record_vix_spike_ic_trade(record: dict) -> None:
    """VIXスパイク翌日のIC実績を vix_spike_trades.jsonl に追記する。

    Args:
        record: dict with keys: date, symbol, event, pnl_usd, vix, delta_pct, signal_id 等
    """
    try:
        VIX_SPIKE_TRADES_FILE.parent.mkdir(parents=True, exist_ok=True)
        record.setdefault("ts", datetime.datetime.now(ET).isoformat())
        record.setdefault("tactic", "ic_sell_vix_spike")
        with open(VIX_SPIKE_TRADES_FILE, "a", encoding="utf-8") as f:
            f.write(json.dumps(record, ensure_ascii=False) + "\n")
    except Exception as e:
        log.warning(f"[VIXSpike30] record_vix_spike_ic_trade failed: {e}")
...
def save_vix_spike_data(vix: float, spike_for_tomorrow: bool):
    ...
    try:
        ...
        _atomic_json_write(VIX_SPIKE_FILE, {
...
def save_ivr_cache(ivr: float):
    ...
    try:
        ...
        _atomic_json_write(IVR_CACHE_FILE, {
            "latest": new_entry,
            "history": history,
        })
    except Exception as e:
...
def fetch_events_weekly():
    ...
    try:
        ...
        _atomic_json_write(EVENTS_FILE, {"fetched": now.date().isoformat(), "events": events})
...
class TradeEngine:
    def __init__(self, paper: bool = False):
        ...
        # B1-CRITICAL: CS発注済みlegのside記録（[sell_side, buy_side]）
        # place_credit_spread で更新・反転決済後にリセット
    ...
    def place_credit_spread(self, sell_code: str, buy_code: str,
        ...
        """
        # B2-CRITICAL: signal_id は分単位で決定的生成（uuid4撤廃）
        # 同じ発注元情報で再起動しても同一keyになりidempotency有効
        if signal_id is None:
            _sell_sym = _extract_symbol_from_code(sell_code) or "UNK"
            _ts_min = datetime.datetime.now(ET).strftime("%Y%m%d%H%M")
            signal_id = f"cs_{direction}_{_sell_sym}_{_ts_min}"
            log.debug(f"[CS] signal_id自動生成(決定的): {signal_id}")
        if DRY_TEST:
        ...
        for leg_idx, (code, side, label, init_price) in enumerate(legs):
            ...
            if order_id is not None:
                ...
                # B1-CRITICAL: 発注成功legのsideを記録（leg_idx 0=sell, 1=buy）
        ...
        if sell_fill is None or buy_fill is None:
            ...
            # B1-CRITICAL: _legs_placed_sides は発注成功時に記録済み（hasattr不要）
            ...
            self._legs_placed_sides = [None, None]  # B1: 反転完了後リセット
            pushover(
                "[Atlas] CS片脚未約定",
    ...
    def cancel_all_open_orders(self, reason: str = "eod_sweep") -> int:
        ...
        if ret != RET_OK or df is None or df.empty:
            ...
            return 0
        # [M-9修正] FILLED_PART除外: 部分約定済みLegをキャンセルすると
        # 約定済み部分は残存し裸ポジション（naked）になるリスクがある。
        # FILLED_PART オーダーは別途手動確認・Pushover通知対象とする。
        ...
        for _, row in df.iterrows():
            ...
            if status in alert_status:
                ...
                log.warning(
                    f"[SweepCancel/{reason}] FILLED_PART検出: oid={oid} "
                    f"code={row.get('code')} dealt={row.get('dealt_qty')}/{row.get('qty')}"
                    f" → キャンセル対象外・手動確認要"
                )
                continue
        ...
        # FILLED_PART発見時: priority=2 Pushover通知
        if filled_part_orders:
            try:
                ...
                pushover_alert(
                    f"[Atlas/ALERT] FILLED_PART検出・手動確認要 ({reason})",
                    f"部分約定オーダーが{len(filled_part_orders)}件あります。\n"
                    f"残存部分が裸ポジションになるリスクあり。\n{_fp_details}",
                    priority=2,
                )
            except Exception:
                log.exception("[spy_bot] FILLED_PART Pushover通知失敗")
        if canceled > 0:
            try:
...
class SMADirectionDetector:
    ...
    def _get_sma20(self) -> Optional[float]:
        ...
        if sma20 is not None:
            ...
            _atomic_json_write(SMA_CACHE_FILE, {
                "date": datetime.datetime.now(ET).strftime("%Y-%m-%d"),
                "sma20": sma20,
            })
        return sma20
    ...
    def _try_delta_hedge(self, greeks: dict):
        ...
        try:
            ...
            try:
                ...
                # [M-4修正] ATMストライク算出: 銘柄別strike_intervalを使って丸める
                # 旧: round(underlying_price) → IWM $0.5/SPX $5刻みを無視して非存在strikeを発注
                # 新: get_symbol_meta()からstrike_intervalを取得して銘柄別に正確なstrikeに丸める
                _hedge_interval = get_symbol_meta(_hedge_underlying).get("strike_interval") or 1.0
                atm_strike = round(underlying_price / _hedge_interval) * _hedge_interval
                ...
                if self.eng:
                    ...
                    # [M-5修正] ヘッジ発注もpre_trade_gateを通す
                    # init_price=Noneだとest_margin=0でmarginチェックが無効化される問題を修正
...
def premarket_assessment(mkt: MarketData, vix: float,
    ...
    # ── VIXスパイク30%翌日バイアス (IC売りIV Crush狙い撃ち) ─────────────────────
    # CBOE実データ (2010-2024, n=47): VIX単日+30%超急騰翌日 平均VIX低下率 -18.3%
    # VIX<40 かつ 前日+30%スパイクが確認 → IC売り優先バイアスを付与
    _vix_spike_30 = is_vix_spike_30_day()
    result["vix_spike_ic_bias"] = _vix_spike_30
    if _vix_spike_30:
        log.info(
            "[VIXSpike30] premarket: 前日VIXスパイク+30%検知 → IC売り優先バイアスON "
            f"(size={VIX_SPIKE_IC_SIZE_FACTOR}x, VIX_MAX={VIX_SPIKE_IC_DISABLE_ABOVE})"
        )
...
def _orb_append_pnl(record: dict):
    ...
    try:
        ...
        trades.append(record)
        _atomic_json_write(ORB_PNL_FILE, {"trades": trades})
    except Exception as e:
...
class ORBPosition:
    ...
    def check_exit(self, current_price: float) -> Optional[str]:
        ...
        1DTE版 (_is_1dte=True) は BT最適値 TP+30%/SL-50% を使用。
        0DTE版は従来通り TP+100%/SL-50%。
        """
        pnl_pct = (current_price - self.entry_price) / self.entry_price
        is_1dte = getattr(self, "_is_1dte", False)
        tp_pct = 0.30 if is_1dte else ORB_TP_PCT
        sl_pct = -0.50   # 両バージョン共通
        if pnl_pct >= tp_pct:
...
class ORBEngine:
    ...
    # 初回利用時にFinnhubから取得し、以後はこのキャッシュを使う。
    # 取得失敗時のみハードコード値にフォールバックする（安全策として残す）。
    _FALLBACK_PRICE_DEFAULTS: dict = {
        "SPY": 560.0, "QQQ": 480.0, "IWM": 200.0,
        "TSLA": 250.0, "NVDA": 900.0, "AAPL": 200.0,
        "MSFT": 420.0, "AMZN": 200.0, "META": 600.0,
        "GOOGL": 170.0,
    }
    _fallback_price_cache: dict = {}       # 起動時Finnhub取得値
    _fallback_cache_fetched: bool = False  # 取得試行済みフラグ

    @classmethod
    def _get_fallback_price(cls, ticker: str) -> float:
        """フォールバック価格を返す。

        初回呼び出し時にFinnhubから全対象銘柄の価格を取得してキャッシュする。
        取得失敗時はハードコードのデフォルト値を使う。
        """
        if not cls._fallback_cache_fetched:
            cls._fallback_cache_fetched = True
            try:
                import requests as _req
                for _t in list(cls._FALLBACK_PRICE_DEFAULTS.keys()):
                    try:
                        _r = _req.get(
                            "https://finnhub.io/api/v1/quote",
                            params={"symbol": _t, "token": FINNHUB_API_KEY},
                            timeout=5,
                        )
                        _p = float(_r.json().get("c") or 0)
                        if _p > 0:
                            cls._fallback_price_cache[_t] = _p
                            log.info(f"[ORB] fallback_price updated: {_t}=${_p:.2f}")
                    except Exception as _fe:
                        log.debug(f"[ORB] fallback_price fetch failed {_t}: {_fe}")
            except Exception as _e:
                log.warning(f"[ORB] fallback_price 一括取得失敗: {_e}")
        # キャッシュ優先、なければデフォルト値
        return cls._fallback_price_cache.get(ticker, cls._FALLBACK_PRICE_DEFAULTS.get(ticker, 300.0))
    ...
    def record_opening_range(self) -> bool:
        ...
        if self.dry_test:
            try:
                ...
                # [M-8修正] fallback_pricesハードコード廃止 → ORBEngine._get_fallback_price()使用
                underlying_price = float(resp.json().get("c") or 0) or self._get_fallback_price(_ticker)
            except Exception:
                underlying_price = self._get_fallback_price(_ticker)
    ...
    def _get_underlying_price(self) -> Optional[float]:
        ...
        if self.dry_test:
            try:
                ...
                # [M-8修正] fallback_pricesハードコード廃止 → ORBEngine._get_fallback_price()使用
                return p if p > 0 else self._get_fallback_price(_ticker)
            except Exception:
                return self._get_fallback_price(_ticker)
    ...
    def _execute_entry_impl(self, direction: str, signal_id: Optional[str] = None) -> Optional[ORBPosition]:
        ...
        if _is_past_entry_cutoff(dry_test=self.dry_test):
            _now_str = datetime.datetime.now(ET).strftime("%H:%M") if not self.dry_test else "dry_test"
            log.info(f"[ORB] execute_entry: {LAST_ENTRY_H}:{LAST_ENTRY_M:02d} ET以降 ({_now_str}) → エントリー中止")
            self.trade_done = True
            return None
        # [M-10修正] WHITELIST方式: symbol_meta.is_allowed()で銘柄を検証
        # ORBEngineにはEXCLUDED_SYMBOLSプロパティがなくwhitelist外銘柄を通過させていた
        _orb_underlying = self.mkt.underlying_code if self.mkt else "US.SPY"
        if not _sym_is_allowed(_orb_underlying):
            log.warning(
                f"[ORB] {_orb_underlying} はALLOWED_SYMBOLS外 → スキップ"
                f" (whitelist方式: symbol_meta.ALLOWED_SYMBOLSで検証)"
            )
            self.trade_done = True
            return None
    ...
    def execute_entry_1dte(self, direction: str, signal_id: Optional[str] = None) -> Optional['ORBPosition']:
        """翌営業日満期 (1DTE) ORB エントリー。

        BT結果 (SPY 135日, WR60%/Sharpe2.49/DD5.2% PASS) に基づく設計:
          - delta 0.40 OTM (0DTE ATM delta=0.50 より低プレミアム・gamma維持)
          - TP +30% / SL -50% (グリッドサーチ最適値)
          - 翌営業日満期: theta decay がマイルド (1日分残る)

        PDT制限下でのday trade回避を主目的として strategy_selector が選択する。
        """
        _orb_orig_underlying = (self.mkt.underlying_code if self.mkt else UNDERLYING_CODE)
        try:
            return self._execute_entry_1dte_impl(direction, signal_id=signal_id)
        finally:
            if self.mkt and self.mkt.underlying_code != _orb_orig_underlying:
                log.info(
                    f"[ORB1DTE] underlying_code復元: {self.mkt.underlying_code} → "
                    f"{_orb_orig_underlying}"
                )
                self.mkt.underlying_code = _orb_orig_underlying

    def _execute_entry_1dte_impl(self, direction: str, signal_id: Optional[str] = None) -> Optional['ORBPosition']:
        """execute_entry_1dte の実装本体。"""
        # エントリーカットオフ (共通)
        if _is_past_entry_cutoff(dry_test=self.dry_test):
            _now_str = datetime.datetime.now(ET).strftime("%H:%M") if not self.dry_test else "dry_test"
            log.info(f"[ORB1DTE] {LAST_ENTRY_H}:{LAST_ENTRY_M:02d} ET以降 ({_now_str}) → エントリー中止")
            self.trade_done = True
            return None

        _orb_underlying = self.mkt.underlying_code if self.mkt else "US.SPY"
        if not _sym_is_allowed(_orb_underlying):
            log.warning(f"[ORB1DTE] {_orb_underlying} はALLOWED_SYMBOLS外 → スキップ")
            self.trade_done = True
            return None
        ...
        underlying_price = self._get_underlying_price()
        if not underlying_price or underlying_price <= 0:
            log.error(f"[ORB1DTE] {_orb_ticker} 価格取得失敗")
            return None
        ...
        log.info(
            f"[ORB1DTE] Entry: {_orb_ticker}={underlying_price:.2f} ATM={atm_strike} "
            f"direction={direction} expiry={expiry_1dte} delta_target=0.40"
        )

        # dry-testモード
        if self.dry_test:
            virtual_price = 2.00   # 1DTE は 0DTE より premium 高め
            virtual_code = (
                f"US.{_orb_ticker}{next_day.strftime('%y%m%d')}"
                f"{'C' if direction == 'CALL' else 'P'}{int(atm_strike * 1000)}"
            )
            cash = 10000.0
            qty = self._calc_qty_1dte(cash, virtual_price)
            log.info(f"[ORB1DTE][DRY-TEST] {_orb_ticker} {direction} {virtual_code} x{qty} @ ${virtual_price:.2f}")
            pushover(
                "[Atlas][ORB1DTE][DRY-TEST]",
                f"{_orb_ticker} 1DTE エントリー: {direction} ATM={atm_strike} x{qty} @ ${virtual_price:.2f}",
            )
            pos = ORBPosition(virtual_code, qty, virtual_price,
                              direction, self.orb_high or underlying_price, self.orb_low or underlying_price)
            _orb_append_pnl({"event": "entry", "tactic": "orb_1dte", "direction": direction,
                             "code": virtual_code, "strike": atm_strike,
                             "qty": qty, "entry_price": virtual_price,
                             "expiry": expiry_1dte, "symbol": _orb_ticker, "vix": vix})
            return pos

        # 1DTE チェーン取得 (翌営業日 expiry)
        chain = self.mkt.get_option_chain_with_greeks(expiry_1dte, direction, center_strike=float(atm_strike))
        if not chain:
            log.error(f"[ORB1DTE] 翌営業日チェーン取得失敗 ({direction} {expiry_1dte})")
            return None

        # delta 0.40 OTM ターゲット (BT最適値)
        ORB1DTE_DELTA_TARGET = 0.40
        ORB1DTE_DELTA_MIN    = 0.35
        ORB1DTE_DELTA_MAX    = 0.45
        ...
        if opt is None:
            ...
            opt = self.mkt.find_by_strike(chain, float(atm_strike))
        if opt is None:
            log.error("[ORB1DTE] オプション選択失敗")
            return None

        _opt_strike = opt.get("strike_price", 0)
        _deviation = abs(_opt_strike - float(atm_strike)) / max(float(atm_strike), 1.0)
        if _deviation > 0.20:   # 1DTE OTM は 0DTE より少し広め
            log.error(
                f"[ORB1DTE] strike整合性NG: option_strike={_opt_strike} vs "
                f"atm_strike={atm_strike} 乖離={_deviation*100:.1f}% → エントリー中止"
            )
            return None
        ...
        if not option_price or option_price <= 0:
            log.error(f"[ORB1DTE] オプション価格取得失敗: {option_code}")
            return None

        _DEEP_ITM_THRESHOLD = 60.0   # 1DTE は 0DTE より少し広め
        if option_price >= _DEEP_ITM_THRESHOLD:
            log.error(f"[ORB1DTE] deep ITM異常価格 → 発注拒否: ${option_price:.2f}")
            return None
        ...
        if not signal_id:
            _orb_bar_ts = datetime.datetime.now(ET).strftime('%Y%m%d%H%M')
            signal_id = f"orb1dte_{_orb_ticker}_{direction}_{_orb_bar_ts}"

        log.info(f"[ORB1DTE] Option: {option_code} strike={option_strike} "
                 f"delta={opt.get('delta', 0):.3f} price=${option_price:.2f} qty={qty}")
        ...
        if FUTU_AVAILABLE and self.eng and self.eng.trade_ctx:
            ...
            order_id, fill_method = self.eng._place_single_leg(
                code=option_code, side=TrdSide.BUY, qty=qty,
                label=f"ORB1DTE_{direction}",
                init_price=mid_price if use_limit else None,
                use_limit=use_limit,
                signal_id=signal_id,
            )
            if order_id is None:
                log.error("[ORB1DTE] 発注失敗")
                pushover_alert("[ORB1DTE] 発注失敗", f"{direction} {option_code}", priority=1)
                return None
            log.info(f"[ORB1DTE] 発注OK: order_id={order_id}")
        else:
            log.info(f"[ORB1DTE][DRY-RUN] BUY {direction} {option_code} x{qty} @ ${option_price:.2f}")

        # ORBPosition を再利用。TP/SL はインスタンス生成後に 1DTE 値で上書き
        pos = ORBPosition(option_code, qty, option_price,
                          direction, self.orb_high or underlying_price, self.orb_low or underlying_price)
        # 1DTE BT最適値: TP+30% / SL-50% で内部比較用に entry_price を活かす
        # ORBPosition.sl_price/tp_price は ORB_TP/SL_PCT 定数を参照するが
        # check_exit() での pnl_pct 比較はエンジン側で ORB1DTE_TP/SL_PCT を渡す必要がある。
        # 暫定: pos に 1dte フラグを付与して check_exit() で分岐させる。
        pos._is_1dte = True   # type: ignore[attr-defined]
        ...
        _orb_sym_short = self.mkt.underlying_code.replace("US.", "") if self.mkt else "SPY"
        # 1DTE TP/SL の実値を Pushover に表示
        tp_display = option_price * 1.30
        sl_display = option_price * 0.50
        pushover(
            "[Atlas][ORB1DTE]",
            f"{_orb_sym_short} 1DTE エントリー: {direction} Strike={option_strike} x{qty}"
            f" @ ${option_price:.2f}\nexp={expiry_1dte}"
            f" TP=${tp_display:.2f}(+30%) SL=${sl_display:.2f}(-50%)",
        )
        return pos

    def _calc_qty_1dte(self, cash: float, option_price: float) -> int:
        """1DTE版サイズ計算。SL=-50%ベースでケリー・係数を適用する。"""
        kelly_frac = self._kelly_fraction
        risk_pct = kelly_frac if (kelly_frac and kelly_frac > 0) else ORB_MAX_RISK_PCT
        risk = cash * risk_pct * self._vix9d_vvix_factor * self._time_zone_factor
        sl_pct = 0.50   # 1DTE BT最適値
        max_loss_per_contract = option_price * sl_pct * 100
...
class CalendarEngine:
    ...
    def execute_entry(self, spy_price: float, vix: float, signal_id: Optional[str] = None) -> Optional[CalendarPosition]:
        ...
        """
        # H-T1: 15:30 ET エントリーカットオフ
        if _is_past_entry_cutoff(dry_test=self.dry_test):
            log.info(f"[Calendar] execute_entry: {LAST_ENTRY_H}:{LAST_ENTRY_M:02d} ET以降 → エントリー中止")
            return None
        ...
        if not _sym_is_allowed(effective_symbol):
            log.info(f"[Calendar] {effective_symbol} はALLOWED_SYMBOLS未登録 → スキップ")
            return None
...
def _gamma_scalp_append_pnl(record: dict):
    try:
        ...
        trades.append(record)
        _atomic_json_write(GAMMA_SCALP_PNL_FILE, {"trades": trades})
    except Exception as e:
...
class StraddleEngine:
    ...
    def execute_entry(self) -> Optional['StraddlePosition']:
        ...
        # H-T1: 15:30 ET エントリーカットオフ
        if _is_past_entry_cutoff(dry_test=self.dry_test):
            log.info(f"[GammaScalp] execute_entry: {LAST_ENTRY_H}:{LAST_ENTRY_M:02d} ET以降 → エントリー中止")
            return None
        import math as _math
...
def _ic_sell_append_pnl(record: dict):
    try:
        ...
        trades.append(record)
        _atomic_json_write(IC_SELL_PNL_FILE, {"trades": trades})
    except Exception as e:
...
def _straddle_buy_append_pnl(record: dict):
    try:
        ...
        trades.append(record)
        _atomic_json_write(STRADDLE_BUY_PNL_FILE, {"trades": trades})
    except Exception as e:
...
class StraddleBuyEngine:
    ...
    def execute_entry(self) -> Optional[StraddleBuyPosition]:
        ...
        # H-T1: 15:30 ET エントリーカットオフ
        if _is_past_entry_cutoff(dry_test=self.dry_test):
            log.info(f"[STRADDLE_BUY] execute_entry: {LAST_ENTRY_H}:{LAST_ENTRY_M:02d} ET以降 → エントリー中止")
            return None
        ...
        if FUTU_AVAILABLE and self.eng and self.eng.trade_ctx:
            ...
            _sb_signal_id = f"straddlebuy_{_sb_sym}_{_sb_ts}"
    ...
    def _get_underlying_price(self) -> Optional[float]:
        ...
        if self.dry_test:
            try:
                ...
                p = float(resp.json().get("c") or 0)
                # [M-8修正] fallback_pricesハードコード廃止 → ORBEngine._get_fallback_price()使用
                return p if p > 0 else ORBEngine._get_fallback_price(sym_ticker)
            except Exception:
                return ORBEngine._get_fallback_price(sym_ticker)
...
class IVCrushEngine:
    ...
    def _execute_entry(self):
        ...
        if self._today_earnings_info is None:
            return False
        # H-T1: 15:30 ET エントリーカットオフ（IVCrushは15:00-15:30が正常窓だが念のため二重確認）
        if _is_past_entry_cutoff(dry_test=self.dry_test):
            log.info(f"[IVCrush] _execute_entry: {LAST_ENTRY_H}:{LAST_ENTRY_M:02d} ET以降 → エントリー中止")
            return False
    ...
    def _record_pnl(self, event, pnl, symbol, strike, qty, reason):
        try:
            ...
            _atomic_json_write(IV_CRUSH_PNL_FILE, existing)
        except Exception as e:
...
class StrangleSellEngine:
    ...
    def should_trade_today(
        ...
        if not ENABLE_STRANGLE_SELL:
            return False
        # H-14修正: EXCLUDED_SYMBOLS廃止 -> ALLOWED_SYMBOLS参照
        if not _sym_is_allowed(symbol):
            log.info(f"[StrangleSell] {symbol} はALLOWED_SYMBOLS未登録 → スキップ")
            return False
        if vix is None:
            return False
        if paper:
    ...
    def execute_entry(self, underlying_price: float,
        ...
        # H-T1: 15:30 ET エントリーカットオフ
        if _is_past_entry_cutoff(dry_test=self.dry_test):
            log.info(f"[StrangleSell] execute_entry: {LAST_ENTRY_H}:{LAST_ENTRY_M:02d} ET以降 → エントリー中止")
            return None
        # H-14修正: EXCLUDED_SYMBOLS廃止 -> ALLOWED_SYMBOLS参照
        if not _sym_is_allowed(self.symbol):
            log.info(f"[StrangleSell] {self.symbol} はALLOWED_SYMBOLS未登録 → スキップ")
            return None
    ...
    def _record_pnl(self, event: str, pnl: float,
        ...
        try:
            ...
            _atomic_json_write(STRANGLE_SELL_PNL_FILE, existing)
        except Exception as e:
...
class IronCondorSellEngine:
    ...
    def __init__(self, mkt, eng, paper=False, dry_test=False):
        ...
        self.entry_done  = False
        self._assessment = None
        self._vix_spike_30 = False   # VIXスパイク+30%翌日フラグ
    ...
    def premarket_check(self):
        ...
        # VIXスパイク30%翌日チェック: サイズ縮小・IVR閾値緩和・実績別集計
        _spike_30 = is_vix_spike_30_day()
        self._vix_spike_30 = _spike_30
        if _spike_30:
            log.info(
                f"[IC_SELL][VIXSpike30] スパイク翌日モード: "
                f"VIX={vix:.1f} IVR={ivr_pct:.0f}%ile "
    ...
    def execute_entry(self, bot=None, signal_id=None, key_levels=None):
        ...
        # -- pre_trade_check (全4層) -------------------------------------------
        # H-8修正: IC max_lossは片翼のみITMのため * 2 は誤り。spread_width * 100 * qty が正しい
        est_margin = spread_width * 100 * qty
        # H-9修正: open_margin_total / symbol_margin を実値で取得してゲートに渡す
        _ic_open_positions_list = self.eng.get_open_positions() if self.eng else []
        open_pos = len(_ic_open_positions_list)
        open_margin_total_ic = sum(
            abs(float(p.get("market_val", 0) or 0))
            for p in _ic_open_positions_list
        )
        for _opt, _side_str in [(call_sell_opt, "SELL"), (put_sell_opt, "SELL")]:
            _ic_sym_margin = sum(
                abs(float(p.get("market_val", 0) or 0))
                for p in _ic_open_positions_list
                if _extract_symbol_from_code(str(p.get("code", ""))) == _extract_symbol_from_code(_opt["code"])
            )
            _allow, _reason = _pre_trade_gate(
                ...
                open_margin_total=open_margin_total_ic,
                symbol_margin=_ic_sym_margin,
                paper=self.paper,
...
def _butterfly_append_pnl(record: dict):
    try:
        ...
        trades.append(record)
        _atomic_json_write(BUTTERFLY_PNL_FILE, {"trades": trades})
    except Exception as e:
...
class ButterflyEngine:
    ...
    def should_trade_today(
        ...
        # H-14修正: EXCLUDED_SYMBOLS廃止 -> ALLOWED_SYMBOLS参照
        if not _sym_is_allowed(symbol):
            log.info(f"[Butterfly] {symbol} はALLOWED_SYMBOLS未登録 -> スキップ")
            return False
        if paper:
    ...
    def _choose_wing_type(self, symbol: str) -> Optional[str]:
        ...
        try:
            ...
            if SMA_CACHE_FILE.exists():
                ...
                # H-13修正: 別銘柄(SPY)のSMAでfallbackするのはAtlas規律違反。
                # 対象銘柄のSMAがなければButterflyエントリー中止（デフォルトCALLへ fallthrough しない）
                sma_val  = sma_data.get(sym_key)
                if sma_val is None and sym_key != "SPY":
                    log.info(f"[Butterfly] {symbol} のSMAキャッシュ未存在 -> エントリー中止")
                    return None  # H-13: SPY fallback禁止・skip
    ...
    def _execute_entry(self, symbol: str, dry_test: bool = False) -> bool:
        ...
        """
        # H-T1: 15:30 ET エントリーカットオフ
        if _is_past_entry_cutoff(dry_test=dry_test):
            log.info(f"[Butterfly] _execute_entry: {LAST_ENTRY_H}:{LAST_ENTRY_M:02d} ET以降 → エントリー中止")
            return False
        ...
        # H-13修正: SMAキャッシュ未存在時はNone返却 → エントリー中止
        if wing_type is None:
            log.info(f"[Butterfly] {symbol} wing_type=None (SMA未取得) -> エントリー中止")
            return False
        ...
        # pre_trade_check 全4層 — H-10修正: 3レグ各leg毎にcheck_order・margin累積
        _bf_open_positions_list = self.eng.get_open_positions() if self.eng else []
        _bf_open_pos     = len(_bf_open_positions_list)
        _bf_margin_total = sum(
            abs(float(p.get("market_val", 0) or 0))
            for p in _bf_open_positions_list
        )
        _bf_sym_margin   = sum(
            abs(float(p.get("market_val", 0) or 0))
            for p in _bf_open_positions_list
            if _extract_symbol_from_code(str(p.get("code", ""))) == symbol
        )
        # 各レグのcheck_order: margin累積で後ろのレグほど厳しい判定になる
        _bf_leg_specs = [
            (lower_code, atm_strike - wing_width, "BUY",  qty,     lower_price, lower_price, lower_price),
            (upper_code, atm_strike + wing_width, "BUY",  qty,     upper_price, upper_price, upper_price),
            (atm_code,   atm_strike,              "SELL", qty * 2, atm_price,   atm_price,   atm_price),
        ]
        _bf_cumulative_margin = _bf_margin_total
        for _bf_code, _bf_strike, _bf_side, _bf_qty, _bf_price, _bf_bid, _bf_ask in _bf_leg_specs:
            _bf_leg_margin = _bf_price * _bf_qty * 100.0
            _bf_ctx = OrderContext(
                symbol=symbol,
                strike=_bf_strike,
                side=_bf_side,
                qty=_bf_qty,
                option_price=_bf_price,
                bid=_bf_bid,
                ask=_bf_ask,
                est_margin=_bf_leg_margin,
                capital_usd=cash_usd,
                open_positions=_bf_open_pos,
                open_margin_total=_bf_cumulative_margin,
                symbol_margin=_bf_sym_margin,
                paper=self.paper,
            )
            _bf_result = check_order(_bf_ctx)
            if not _bf_result.allow:
                log.warning(
                    f"[Butterfly] pre_trade_check 拒否 leg={_bf_code} "
                    f"[{_bf_result.layer}]: {_bf_result.reason}"
                )
                return False
            _bf_cumulative_margin += _bf_leg_margin
...
class SPYCreditSpreadBot:
    ...
    def _try_mass_verify_entry(self, symbol: str, tactic: str, key: str) -> None:
        ...
        try:
            ...
            try:
                ...
                elif tactic == "orb_1dte" and self.orb_engine is not None:
                    # ── ORB 1DTE 戦術 ─────────────────────────────────────
                    # BT: SPY 70T / WR60% / Sharpe2.49 / DD5.2% PASS (135日)
                    # delta0.40 OTM / TP+30% / SL-50% / 翌営業日満期
                    _orb1dte_dir = self.orb_engine.check_breakout()
                    if _orb1dte_dir is None:
                        if DRY_TEST:
                            _orb1dte_dir = "CALL"
                        else:
                            log.debug(
                                f"[MassVerify] {symbol.replace('US.','')}×orb_1dte "
                                f"ブレイクアウトなし → スキップ"
                            )
                            return
                    _orb1dte_pos = self.orb_engine.execute_entry_1dte(_orb1dte_dir)
                    if _orb1dte_pos:
                        self._mass_verify_positions[key] = {
                            "tactic": "orb_1dte",
                            "position": _orb1dte_pos,
                            "engine": "orb",
                            "entry_time": datetime.datetime.now(ET).isoformat(),
                            "symbol": symbol,
                        }
                        append_pnl_entry({
                            "event": "entry",
                            "strategy": "mass_verify_orb_1dte",
                            "symbol": symbol.replace("US.", ""),
                            "tactic": "orb_1dte",
                            "mass_verify_key": key,
                            "direction": _orb1dte_dir,
                        })
                        log.info(
                            f"[MassVerify] {symbol.replace('US.','')}×orb_1dte "
                            f"エントリー完了 direction={_orb1dte_dir}"
                        )
                    else:
                        log.debug(
                            f"[MassVerify] {symbol.replace('US.','')}×orb_1dte "
                            f"execute_entry_1dte失敗 → スキップ"
                        )
    ...
    def _check_mass_verify_exit(self, symbol: str, tactic: str, key: str) -> None:
        ...
        try:
            ...
            # ── ORB 1DTE: check_exit委譲 (TP+30%/SL-50% は _is_1dte フラグで分岐) ──
            elif _tactic == "orb_1dte" and self.orb_engine:
                _mv1_pos = pos_info.get("position")
                if _mv1_pos:
                    _orig1 = self.mkt.underlying_code
                    try:
                        self.mkt.underlying_code = _sym
                        _mv1_price = self.orb_engine._get_option_price(_mv1_pos)
                        if _mv1_price is not None:
                            _exit1_reason = _mv1_pos.check_exit(_mv1_price)
                            if _exit1_reason:
                                _exit1_result = self.orb_engine._close_position(
                                    _mv1_pos, _mv1_price, _exit1_reason
                                )
                                _pnl1 = _exit1_result.get("pnl_usd", 0) if _exit1_result else 0.0
                                append_pnl_entry({
                                    "event": "exit",
                                    "strategy": "mass_verify_orb_1dte",
                                    "symbol": _sym.replace("US.", ""),
                                    "tactic": "orb_1dte",
                                    "mass_verify_key": key,
                                    "reason": _exit1_reason,
                                    "pnl_usd": round(_pnl1, 2),
                                })
                                log.info(
                                    f"[MassVerify] {_sym.replace('US.','')}×orb_1dte "
                                    f"決済完了 reason={_exit1_reason} pnl=${_pnl1:+.2f}"
                                )
                                del self._mass_verify_positions[key]
                    finally:
                        self.mkt.underlying_code = _orig1
    ...
    def compute_next_day_bias(self):
        ...
        try:
            ...
            _atomic_json_write(NEXT_DAY_BIAS_FILE, result, ensure_ascii=False)

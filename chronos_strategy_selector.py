#!/usr/bin/env python3
"""
chronos_strategy_selector.py — MFFU先物向け 戦術選択エンジン
...
def select_futures_strategy(env: dict) -> list[dict]:
    ...
    if _in_us_session:
        ...
        # ── Volume Profile ───────────────────────────────────────────────────
        # vp_profile は env から渡す（chronos_bot.pyで事前算出して env に含める）。
        # 未提供の場合はスキップ。
        ...
        # ── Range Break 改良版 ───────────────────────────────────────────────
        # donchian_channel は env から渡す（chronos_bot.pyで事前算出）。
        # 未提供の場合はスキップ。
    ...
    if _ES_NQ_SPREAD_AVAILABLE:
        ...
        # 実際の発注・ポジション管理は chronos_bot.py の self.es_nq_spread が担う。
    ...
    if _LEVEL_TRADING_AVAILABLE and _in_us_session:
        ...
        if lv_levels and lv_price and lv_ib_done:
            try:
                ...
                # chronos_bot.py の self.level_trading とは独立したインスタンス（発注なし）。
...
def build_env_dict(
    ...
    chronos_bot.py の run_forever() ループから呼ぶ用。

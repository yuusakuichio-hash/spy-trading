#!/usr/bin/env python3
"""
mffu_bot.py — MyFundedFutures (MFFU) 先物自動売買Bot v1
...
設計方針:
  ...
  Atlas基盤実流用 (実質約30%):
    ...
    mffu_strategy_selector.select_futures_strategy() — 先物専用セレクター（新規）
...
from mffu_rule_simulator import (
    MFFU_ACCOUNT_RULES,
    check_eod_drawdown,
...
class MFFUBot:
    ...
    def select_strategies(
        ...
        if not MFFU_SELECTOR_AVAILABLE:
            log.warning("[MFFUBot] mffu_strategy_selector not available: fallback to ORB only")

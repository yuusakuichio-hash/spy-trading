...
**採点日時**: 2026-04-19T18:03:20.402824
...
**実装エビデンス**:
- `chronos_bot.py:824` — `class FuturesORBStrategy:`
- `chronos_bot.py:879` — `def update_or_candle(self, high: float, low: float):`
- `chronos_bot.py:886` — `def finalize_or(self):`
- `chronos_bot.py:900` — `def check_breakout(`
...
**実装エビデンス**:
- `chronos_bot.py:432` — `class NewsTradingFilter:`
- `chronos_rules.yaml:207` — `blackout_window_sec: 120`
- `chronos_rules.yaml:210` — `t1_events:`
- `chronos_rules.yaml:211` — `- "FOMC"`
- `chronos_bot.py:919` — `news_check = self.news_filter.is_blackout(now_et)`
...
**実装エビデンス**:
- `chronos_bot.py:1685` — `def _is_maintenance_break(self, now_et: Optional[datetime.datetime] = None) -> bool:`
- `chronos_rules.yaml:231` — `# CME Globex 毎日 ET 17:00-18:00 の清算窓（市場閉場）`
- `chronos_rules.yaml:240` — `block_new_orders: true`
- `chronos_bot.py:2442` — `if self._is_maintenance_break(now_et):`
...
- `chronos_pre_trade_check.py:50` — `# MES と ES は実質同一プロダクト (S&P500 先物) のため両建て禁止`
- `chronos_bot.py:988` — `# place_order 直前で check_hedging_violation() を必ず通過させる。`
...
- `chronos_bot.py:1354` — `self._consecutive_losses:   int  = 0       # 連続負け数（日次リセット）`
- `chronos_bot.py:1807` — `def record_trade_result(self, pnl: float) -> None:`
- `chronos_bot.py:1353` — `# 2連敗→50%, 3連敗→25%, 5連敗→当日停止`
- `chronos_bot.py:3102` — `# 連敗カウンタ日次リセット (consecutive_loss_guard: daily_reset_et="09:00")`
...
- `chronos_bot.py:1643` — `gf_atr_5d: float = 10.0  # フォールバック値`
- `chronos_bot.py:1654` — `gf_atr_5d = sum(ranges) / len(ranges)`
...
- `chronos_bot.py:1528` — `self.vwap = None`
...
- `test_f12_f13_critical_fixes_20260419.py:0` — `[PYTEST] F12 tests: passed=35 failed=0`
- `chronos_bot.py:3099` — `self.cumulative_delta.daily_reset()`
- `chronos_bot.py:1539` — `env_dict["cumulative_delta_bias"] = _cd_bias`
...
- `test_f12_f13_critical_fixes_20260419.py:0` — `[PYTEST] F13 tests: passed=35 failed=0`
...
- `chronos_bot.py:1357` — `# ── MVP追加: Phase / account_type 管理 ──────────────────────────────────`
- `chronos_bot.py:2103` — `def on_payout_received(self) -> bool:`
- `chronos_bot.py:1380` — `# MFFU: 初回ペイアウト後 MLL $100 → survival_mode_after_payout 適用`
...
- `chronos_bot.py:1088` — `contracts = floor(dollar_risk / risk_per_contract)`
- `chronos_bot.py:1129` — `max_daily_pnl = max(monthly_realized_pnl, monthly_target) * 0.35`
...
*Generated by scripts/futures_trader_evaluation.py (Sora Lab / Chronos) — 2026-04-19 18:03 JST*

...
- **Generated**: 2026-04-21T07:00:26
- **Source**: fallback
- **Task**: SNS daily post for 2026-04-21: read condor_pnl.json, extract 2026-04-20 ET exit events, generate factual post text under 268 X chars, call post_to_x() from sns_daily_post.py, save daily_20260421.txt, Pushover report
- **Files**: data/condor_pnl.json, sns_daily_post.py, data/sns/
- **Overall Risk**: medium
...
### 詳細
- **F01 依存サービス未起動でフェイル**: SNS daily post for 2026-04-21: read condor_pnl.json, extract... に対し『依存サービス未起動でフェイル』が発生し、復旧不能または無音で不正動作する。
- **F02 API rate limit / auth 失敗**: SNS daily post for 2026-04-21: read condor_pnl.json, extract... に対し『API rate limit / auth 失敗』が発生し、復旧不能または無音で不正動作する。
- **F03 データ型/スキーマ不整合で silent fail**: SNS daily post for 2026-04-21: read condor_pnl.json, extract... に対し『データ型/スキーマ不整合で silent fail』が発生し、復旧不能または無音で不正動作する。
- **F04 タイムゾーン混在 (JST/ET/UTC)**: SNS daily post for 2026-04-21: read condor_pnl.json, extract... に対し『タイムゾーン混在 (JST/ET/UTC)』が発生し、復旧不能または無音で不正動作する。
- **F05 並行実行で race condition**: SNS daily post for 2026-04-21: read condor_pnl.json, extract... に対し『並行実行で race condition』が発生し、復旧不能または無音で不正動作する。
- **F06 ディスク/メモリ枯渇**: SNS daily post for 2026-04-21: read condor_pnl.json, extract... に対し『ディスク/メモリ枯渇』が発生し、復旧不能または無音で不正動作する。
- **F07 冬時間/夏時間境界で off-by-one**: SNS daily post for 2026-04-21: read condor_pnl.json, extract... に対し『冬時間/夏時間境界で off-by-one』が発生し、復旧不能または無音で不正動作する。
- **F08 既存ファイル破壊・未バックアップ**: SNS daily post for 2026-04-21: read condor_pnl.json, extract... に対し『既存ファイル破壊・未バックアップ』が発生し、復旧不能または無音で不正動作する。
- **F09 テスト不足で本番初回に発覚**: SNS daily post for 2026-04-21: read condor_pnl.json, extract... に対し『テスト不足で本番初回に発覚』が発生し、復旧不能または無音で不正動作する。
- **F10 roll-back 手順未定義で復旧不能**: SNS daily post for 2026-04-21: read condor_pnl.json, extract... に対し『roll-back 手順未定義で復旧不能』が発生し、復旧不能または無音で不正動作する。

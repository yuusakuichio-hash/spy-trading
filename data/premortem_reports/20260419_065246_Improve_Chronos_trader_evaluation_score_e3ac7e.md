...
- **Generated**: 2026-04-19T06:52:46
- **Source**: fallback
- **Task**: Improve Chronos trader evaluation score from 62/80 to 70+/80 by fixing F10 ATR Regime 2->5, F11 VWAP 3->5, F3 Daily soft stop 4->5, F4 Consistency call_site integration 4->5, F7 Hedging check integration in place_order path 4->5. Must keep existing 231 tests passing and maintain all previous MVP/Survival Mode guards
- **Files**: chronos_bot.py, chronos_strategy_selector.py, chronos_pre_trade_check.py, chronos_rules.yaml
- **Overall Risk**: medium
...
### 詳細
- **F01 依存サービス未起動でフェイル**: Improve Chronos trader evaluation score from 62/80 to 70+/80... に対し『依存サービス未起動でフェイル』が発生し、復旧不能または無音で不正動作する。
- **F02 API rate limit / auth 失敗**: Improve Chronos trader evaluation score from 62/80 to 70+/80... に対し『API rate limit / auth 失敗』が発生し、復旧不能または無音で不正動作する。
- **F03 データ型/スキーマ不整合で silent fail**: Improve Chronos trader evaluation score from 62/80 to 70+/80... に対し『データ型/スキーマ不整合で silent fail』が発生し、復旧不能または無音で不正動作する。
- **F04 タイムゾーン混在 (JST/ET/UTC)**: Improve Chronos trader evaluation score from 62/80 to 70+/80... に対し『タイムゾーン混在 (JST/ET/UTC)』が発生し、復旧不能または無音で不正動作する。
- **F05 並行実行で race condition**: Improve Chronos trader evaluation score from 62/80 to 70+/80... に対し『並行実行で race condition』が発生し、復旧不能または無音で不正動作する。
- **F06 ディスク/メモリ枯渇**: Improve Chronos trader evaluation score from 62/80 to 70+/80... に対し『ディスク/メモリ枯渇』が発生し、復旧不能または無音で不正動作する。
- **F07 冬時間/夏時間境界で off-by-one**: Improve Chronos trader evaluation score from 62/80 to 70+/80... に対し『冬時間/夏時間境界で off-by-one』が発生し、復旧不能または無音で不正動作する。
- **F08 既存ファイル破壊・未バックアップ**: Improve Chronos trader evaluation score from 62/80 to 70+/80... に対し『既存ファイル破壊・未バックアップ』が発生し、復旧不能または無音で不正動作する。
- **F09 テスト不足で本番初回に発覚**: Improve Chronos trader evaluation score from 62/80 to 70+/80... に対し『テスト不足で本番初回に発覚』が発生し、復旧不能または無音で不正動作する。
- **F10 roll-back 手順未定義で復旧不能**: Improve Chronos trader evaluation score from 62/80 to 70+/80... に対し『roll-back 手順未定義で復旧不能』が発生し、復旧不能または無音で不正動作する。

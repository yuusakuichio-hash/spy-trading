...
- **Generated**: 2026-04-22T19:56:44
- **Source**: fallback
- **Task**: chronos_v3 仕様書 v3 起草・11 戦術 × 6 銘柄 + MFFU/Tradeify/Apex プロップ対応 + TradersPost webhook + Tradovate demo + core/strategies/prop/broker 層の Interface 凍結。common_v3 依存
- **Overall Risk**: medium
...
### 詳細
- **F01 Bearer トークン期限切れで全リクエスト401**: chronos_v3 仕様書 v3 起草・11 戦術 × 6 銘柄 + MFFU/Tradeify/Apex プロップ対... に対し『Bearer トークン期限切れで全リクエスト401』が発生し、復旧不能または無音で不正動作する。
- **F02 エンドポイントURL変更で silent 404**: chronos_v3 仕様書 v3 起草・11 戦術 × 6 銘柄 + MFFU/Tradeify/Apex プロップ対... に対し『エンドポイントURL変更で silent 404』が発生し、復旧不能または無音で不正動作する。
- **F03 レート制限429で発注ループが詰まる**: chronos_v3 仕様書 v3 起草・11 戦術 × 6 銘柄 + MFFU/Tradeify/Apex プロップ対... に対し『レート制限429で発注ループが詰まる』が発生し、復旧不能または無音で不正動作する。
- **F04 タイムアウト未設定でスレッドが永久ブロック**: chronos_v3 仕様書 v3 起草・11 戦術 × 6 銘柄 + MFFU/Tradeify/Apex プロップ対... に対し『タイムアウト未設定でスレッドが永久ブロック』が発生し、復旧不能または無音で不正動作する。
- **F05 リプレイ攻撃でべき等でない操作が二重実行**: chronos_v3 仕様書 v3 起草・11 戦術 × 6 銘柄 + MFFU/Tradeify/Apex プロップ対... に対し『リプレイ攻撃でべき等でない操作が二重実行』が発生し、復旧不能または無音で不正動作する。
- **F06 TLS証明書期限切れで接続拒否**: chronos_v3 仕様書 v3 起草・11 戦術 × 6 銘柄 + MFFU/Tradeify/Apex プロップ対... に対し『TLS証明書期限切れで接続拒否』が発生し、復旧不能または無音で不正動作する。
- **F07 JSONレスポンスのフィールド名変更で KeyError**: chronos_v3 仕様書 v3 起草・11 戦術 × 6 銘柄 + MFFU/Tradeify/Apex プロップ対... に対し『JSONレスポンスのフィールド名変更で KeyError』が発生し、復旧不能または無音で不正動作する。
- **F08 ネットワーク瞬断でリトライなし→発注欠落**: chronos_v3 仕様書 v3 起草・11 戦術 × 6 銘柄 + MFFU/Tradeify/Apex プロップ対... に対し『ネットワーク瞬断でリトライなし→発注欠落』が発生し、復旧不能または無音で不正動作する。

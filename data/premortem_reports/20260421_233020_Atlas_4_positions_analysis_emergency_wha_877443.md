...
- **Generated**: 2026-04-21T23:30:20
- **Source**: fallback
- **Task**: Atlas 4 positions analysis emergency: what strategy entered long straddle x2, why not credit, trace entry log, quote context failure root cause
- **Overall Risk**: medium
...
### 詳細
- **F01 PDT違反で口座が90日ロック**: Atlas 4 positions analysis emergency: what strategy entered ... に対し『PDT違反で口座が90日ロック』が発生し、復旧不能または無音で不正動作する。
- **F02 資金管理ロジックのバグで証拠金超過発注**: Atlas 4 positions analysis emergency: what strategy entered ... に対し『資金管理ロジックのバグで証拠金超過発注』が発生し、復旧不能または無音で不正動作する。
- **F03 DD上限超過後もエントリー継続で連鎖損失**: Atlas 4 positions analysis emergency: what strategy entered ... に対し『DD上限超過後もエントリー継続で連鎖損失』が発生し、復旧不能または無音で不正動作する。
- **F04 市場クローズ直前に発注→約定できずポジション持ち越し**: Atlas 4 positions analysis emergency: what strategy entered ... に対し『市場クローズ直前に発注→約定できずポジション持ち越し』が発生し、復旧不能または無音で不正動作する。
- **F05 VIX急騰時に戦術切替が遅延し最悪タイミングで発注**: Atlas 4 positions analysis emergency: what strategy entered ... に対し『VIX急騰時に戦術切替が遅延し最悪タイミングで発注』が発生し、復旧不能または無音で不正動作する。
- **F06 二重発注防止ロジックのバグで同一シグナルを複数回発注**: Atlas 4 positions analysis emergency: what strategy entered ... に対し『二重発注防止ロジックのバグで同一シグナルを複数回発注』が発生し、復旧不能または無音で不正動作する。
- **F07 fill 確認なしで損切注文が未執行のまま放置**: Atlas 4 positions analysis emergency: what strategy entered ... に対し『fill 確認なしで損切注文が未執行のまま放置』が発生し、復旧不能または無音で不正動作する。
- **F08 タイムゾーン誤りで場外に発注ループが走る**: Atlas 4 positions analysis emergency: what strategy entered ... に対し『タイムゾーン誤りで場外に発注ループが走る』が発生し、復旧不能または無音で不正動作する。

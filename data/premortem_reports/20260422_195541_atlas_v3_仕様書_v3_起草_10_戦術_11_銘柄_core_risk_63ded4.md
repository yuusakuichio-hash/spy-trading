...
- **Generated**: 2026-04-22T19:55:41
- **Source**: fallback
- **Task**: atlas_v3 仕様書 v3 起草・10 戦術 × 11 銘柄 + core/risk/broker 層の Interface 凍結。common_v3 依存・Phase 2 実装の前提・ bug zero 規律・silent except 禁止
- **Overall Risk**: medium
...
### 詳細
- **F01 PDT違反で口座が90日ロック**: atlas_v3 仕様書 v3 起草・10 戦術 × 11 銘柄 + core/risk/broker 層の Inter... に対し『PDT違反で口座が90日ロック』が発生し、復旧不能または無音で不正動作する。
- **F02 資金管理ロジックのバグで証拠金超過発注**: atlas_v3 仕様書 v3 起草・10 戦術 × 11 銘柄 + core/risk/broker 層の Inter... に対し『資金管理ロジックのバグで証拠金超過発注』が発生し、復旧不能または無音で不正動作する。
- **F03 DD上限超過後もエントリー継続で連鎖損失**: atlas_v3 仕様書 v3 起草・10 戦術 × 11 銘柄 + core/risk/broker 層の Inter... に対し『DD上限超過後もエントリー継続で連鎖損失』が発生し、復旧不能または無音で不正動作する。
- **F04 市場クローズ直前に発注→約定できずポジション持ち越し**: atlas_v3 仕様書 v3 起草・10 戦術 × 11 銘柄 + core/risk/broker 層の Inter... に対し『市場クローズ直前に発注→約定できずポジション持ち越し』が発生し、復旧不能または無音で不正動作する。
- **F05 VIX急騰時に戦術切替が遅延し最悪タイミングで発注**: atlas_v3 仕様書 v3 起草・10 戦術 × 11 銘柄 + core/risk/broker 層の Inter... に対し『VIX急騰時に戦術切替が遅延し最悪タイミングで発注』が発生し、復旧不能または無音で不正動作する。
- **F06 二重発注防止ロジックのバグで同一シグナルを複数回発注**: atlas_v3 仕様書 v3 起草・10 戦術 × 11 銘柄 + core/risk/broker 層の Inter... に対し『二重発注防止ロジックのバグで同一シグナルを複数回発注』が発生し、復旧不能または無音で不正動作する。
- **F07 fill 確認なしで損切注文が未執行のまま放置**: atlas_v3 仕様書 v3 起草・10 戦術 × 11 銘柄 + core/risk/broker 層の Inter... に対し『fill 確認なしで損切注文が未執行のまま放置』が発生し、復旧不能または無音で不正動作する。
- **F08 タイムゾーン誤りで場外に発注ループが走る**: atlas_v3 仕様書 v3 起草・10 戦術 × 11 銘柄 + core/risk/broker 層の Inter... に対し『タイムゾーン誤りで場外に発注ループが走る』が発生し、復旧不能または無音で不正動作する。

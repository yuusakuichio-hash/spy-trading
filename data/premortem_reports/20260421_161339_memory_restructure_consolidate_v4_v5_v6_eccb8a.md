...
- **Generated**: 2026-04-21T16:13:39
- **Source**: fallback
- **Task**: memory restructure: consolidate v4/v5/v6 versioned files, move old versions to archive/2026-04/, create daily/ digest dir, rewrite MEMORY.md index to <150 lines, add 3 new memory files for architecture/update rules/notification policy
- **Overall Risk**: medium
...
### 詳細
- **F01 依存サービス未起動でフェイル**: memory restructure: consolidate v4/v5/v6 versioned files, mo... に対し『依存サービス未起動でフェイル』が発生し、復旧不能または無音で不正動作する。
- **F02 API rate limit / auth 失敗**: memory restructure: consolidate v4/v5/v6 versioned files, mo... に対し『API rate limit / auth 失敗』が発生し、復旧不能または無音で不正動作する。
- **F03 データ型/スキーマ不整合で silent fail**: memory restructure: consolidate v4/v5/v6 versioned files, mo... に対し『データ型/スキーマ不整合で silent fail』が発生し、復旧不能または無音で不正動作する。
- **F04 タイムゾーン混在 (JST/ET/UTC)**: memory restructure: consolidate v4/v5/v6 versioned files, mo... に対し『タイムゾーン混在 (JST/ET/UTC)』が発生し、復旧不能または無音で不正動作する。
- **F05 並行実行で race condition**: memory restructure: consolidate v4/v5/v6 versioned files, mo... に対し『並行実行で race condition』が発生し、復旧不能または無音で不正動作する。
- **F06 ディスク/メモリ枯渇**: memory restructure: consolidate v4/v5/v6 versioned files, mo... に対し『ディスク/メモリ枯渇』が発生し、復旧不能または無音で不正動作する。
- **F07 冬時間/夏時間境界で off-by-one**: memory restructure: consolidate v4/v5/v6 versioned files, mo... に対し『冬時間/夏時間境界で off-by-one』が発生し、復旧不能または無音で不正動作する。
- **F08 既存ファイル破壊・未バックアップ**: memory restructure: consolidate v4/v5/v6 versioned files, mo... に対し『既存ファイル破壊・未バックアップ』が発生し、復旧不能または無音で不正動作する。
- **F09 テスト不足で本番初回に発覚**: memory restructure: consolidate v4/v5/v6 versioned files, mo... に対し『テスト不足で本番初回に発覚』が発生し、復旧不能または無音で不正動作する。
- **F10 roll-back 手順未定義で復旧不能**: memory restructure: consolidate v4/v5/v6 versioned files, mo... に対し『roll-back 手順未定義で復旧不能』が発生し、復旧不能または無音で不正動作する。

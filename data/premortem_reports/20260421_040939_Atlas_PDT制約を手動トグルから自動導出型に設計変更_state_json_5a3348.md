...
- **Generated**: 2026-04-21T04:09:39
- **Source**: fallback
- **Task**: Atlas PDT制約を手動トグルから自動導出型に設計変更。state.jsonのpdt_constrainedフィールド廃止、mode(paper/live)+capital(K threshold)から動的算出、Defense-in-Depthでpre-trade check統合、統合テスト4パターン追加。本番稼働時にPDT無効化する手段が物理的に存在しないことを保証する。
- **Overall Risk**: medium
...
### 詳細
- **F01 依存サービス未起動でフェイル**: Atlas PDT制約を手動トグルから自動導出型に設計変更。state.jsonのpdt_constrainedフィール... に対し『依存サービス未起動でフェイル』が発生し、復旧不能または無音で不正動作する。
- **F02 API rate limit / auth 失敗**: Atlas PDT制約を手動トグルから自動導出型に設計変更。state.jsonのpdt_constrainedフィール... に対し『API rate limit / auth 失敗』が発生し、復旧不能または無音で不正動作する。
- **F03 データ型/スキーマ不整合で silent fail**: Atlas PDT制約を手動トグルから自動導出型に設計変更。state.jsonのpdt_constrainedフィール... に対し『データ型/スキーマ不整合で silent fail』が発生し、復旧不能または無音で不正動作する。
- **F04 タイムゾーン混在 (JST/ET/UTC)**: Atlas PDT制約を手動トグルから自動導出型に設計変更。state.jsonのpdt_constrainedフィール... に対し『タイムゾーン混在 (JST/ET/UTC)』が発生し、復旧不能または無音で不正動作する。
- **F05 並行実行で race condition**: Atlas PDT制約を手動トグルから自動導出型に設計変更。state.jsonのpdt_constrainedフィール... に対し『並行実行で race condition』が発生し、復旧不能または無音で不正動作する。
- **F06 ディスク/メモリ枯渇**: Atlas PDT制約を手動トグルから自動導出型に設計変更。state.jsonのpdt_constrainedフィール... に対し『ディスク/メモリ枯渇』が発生し、復旧不能または無音で不正動作する。
- **F07 冬時間/夏時間境界で off-by-one**: Atlas PDT制約を手動トグルから自動導出型に設計変更。state.jsonのpdt_constrainedフィール... に対し『冬時間/夏時間境界で off-by-one』が発生し、復旧不能または無音で不正動作する。
- **F08 既存ファイル破壊・未バックアップ**: Atlas PDT制約を手動トグルから自動導出型に設計変更。state.jsonのpdt_constrainedフィール... に対し『既存ファイル破壊・未バックアップ』が発生し、復旧不能または無音で不正動作する。
- **F09 テスト不足で本番初回に発覚**: Atlas PDT制約を手動トグルから自動導出型に設計変更。state.jsonのpdt_constrainedフィール... に対し『テスト不足で本番初回に発覚』が発生し、復旧不能または無音で不正動作する。
- **F10 roll-back 手順未定義で復旧不能**: Atlas PDT制約を手動トグルから自動導出型に設計変更。state.jsonのpdt_constrainedフィール... に対し『roll-back 手順未定義で復旧不能』が発生し、復旧不能または無音で不正動作する。

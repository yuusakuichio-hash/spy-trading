...
- **Generated**: 2026-04-19T05:48:38
- **Source**: fallback
- **Task**: Build elite MES futures trader evaluation framework (FW): implement scripts/futures_trader_evaluation.py based on 16 criteria from data/research_mes_trader_day_20260419.md Section 6. Score Chronos current implementation, report gaps, feed into improvement cycle (research -> fix -> 3rd-party verify -> re-score)
...
### 詳細
- **F01 依存サービス未起動でフェイル**: Build elite MES futures trader evaluation framework (FW): im... に対し『依存サービス未起動でフェイル』が発生し、復旧不能または無音で不正動作する。
- **F02 API rate limit / auth 失敗**: Build elite MES futures trader evaluation framework (FW): im... に対し『API rate limit / auth 失敗』が発生し、復旧不能または無音で不正動作する。
- **F03 データ型/スキーマ不整合で silent fail**: Build elite MES futures trader evaluation framework (FW): im... に対し『データ型/スキーマ不整合で silent fail』が発生し、復旧不能または無音で不正動作する。
- **F04 タイムゾーン混在 (JST/ET/UTC)**: Build elite MES futures trader evaluation framework (FW): im... に対し『タイムゾーン混在 (JST/ET/UTC)』が発生し、復旧不能または無音で不正動作する。
- **F05 並行実行で race condition**: Build elite MES futures trader evaluation framework (FW): im... に対し『並行実行で race condition』が発生し、復旧不能または無音で不正動作する。
- **F06 ディスク/メモリ枯渇**: Build elite MES futures trader evaluation framework (FW): im... に対し『ディスク/メモリ枯渇』が発生し、復旧不能または無音で不正動作する。
- **F07 冬時間/夏時間境界で off-by-one**: Build elite MES futures trader evaluation framework (FW): im... に対し『冬時間/夏時間境界で off-by-one』が発生し、復旧不能または無音で不正動作する。
- **F08 既存ファイル破壊・未バックアップ**: Build elite MES futures trader evaluation framework (FW): im... に対し『既存ファイル破壊・未バックアップ』が発生し、復旧不能または無音で不正動作する。
- **F09 テスト不足で本番初回に発覚**: Build elite MES futures trader evaluation framework (FW): im... に対し『テスト不足で本番初回に発覚』が発生し、復旧不能または無音で不正動作する。
- **F10 roll-back 手順未定義で復旧不能**: Build elite MES futures trader evaluation framework (FW): im... に対し『roll-back 手順未定義で復旧不能』が発生し、復旧不能または無音で不正動作する。

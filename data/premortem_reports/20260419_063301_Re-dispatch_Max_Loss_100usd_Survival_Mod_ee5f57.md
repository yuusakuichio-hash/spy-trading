...
- **Generated**: 2026-04-19T06:33:01
- **Source**: fallback
- **Task**: Re-dispatch Max Loss 100usd Survival Mode builder with expression fixed. Implement sim_funded_after_payout phase with max 1 contract, daily loss stop 80usd, per-trade stop 25usd, profit lock 150usd daily terminate, max 1 trade per day, A-plus window only 10:30-11:00 ET, post-loss cooldown 3 business days, required setup score 80+
- **Files**: chronos_bot.py, chronos_rules.yaml, chronos_mffu_rules.py, chronos_strategy_selector.py
- **Overall Risk**: medium
...
### 詳細
- **F01 依存サービス未起動でフェイル**: Re-dispatch Max Loss 100usd Survival Mode builder with expre... に対し『依存サービス未起動でフェイル』が発生し、復旧不能または無音で不正動作する。
- **F02 API rate limit / auth 失敗**: Re-dispatch Max Loss 100usd Survival Mode builder with expre... に対し『API rate limit / auth 失敗』が発生し、復旧不能または無音で不正動作する。
- **F03 データ型/スキーマ不整合で silent fail**: Re-dispatch Max Loss 100usd Survival Mode builder with expre... に対し『データ型/スキーマ不整合で silent fail』が発生し、復旧不能または無音で不正動作する。
- **F04 タイムゾーン混在 (JST/ET/UTC)**: Re-dispatch Max Loss 100usd Survival Mode builder with expre... に対し『タイムゾーン混在 (JST/ET/UTC)』が発生し、復旧不能または無音で不正動作する。
- **F05 並行実行で race condition**: Re-dispatch Max Loss 100usd Survival Mode builder with expre... に対し『並行実行で race condition』が発生し、復旧不能または無音で不正動作する。
- **F06 ディスク/メモリ枯渇**: Re-dispatch Max Loss 100usd Survival Mode builder with expre... に対し『ディスク/メモリ枯渇』が発生し、復旧不能または無音で不正動作する。
- **F07 冬時間/夏時間境界で off-by-one**: Re-dispatch Max Loss 100usd Survival Mode builder with expre... に対し『冬時間/夏時間境界で off-by-one』が発生し、復旧不能または無音で不正動作する。
- **F08 既存ファイル破壊・未バックアップ**: Re-dispatch Max Loss 100usd Survival Mode builder with expre... に対し『既存ファイル破壊・未バックアップ』が発生し、復旧不能または無音で不正動作する。
- **F09 テスト不足で本番初回に発覚**: Re-dispatch Max Loss 100usd Survival Mode builder with expre... に対し『テスト不足で本番初回に発覚』が発生し、復旧不能または無音で不正動作する。
- **F10 roll-back 手順未定義で復旧不能**: Re-dispatch Max Loss 100usd Survival Mode builder with expre... に対し『roll-back 手順未定義で復旧不能』が発生し、復旧不能または無音で不正動作する。

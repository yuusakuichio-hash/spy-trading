...
- **Generated**: 2026-04-24T21:46:13
- **Source**: fallback
- **Task**: ADR-015 A: dead_man_switch.py に OpenD + atlas-paper 死活監視を追加・Pushover P1 通知・unit test 追加
- **Files**: scripts/dead_man_switch.py
- **Overall Risk**: medium
...
### 詳細
- **F01 flaky test がCI環境のみ失敗し本番バグを隠蔽**: ADR-015 A: dead_man_switch.py に OpenD + atlas-paper 死活監視を追加・... に対し『flaky test がCI環境のみ失敗し本番バグを隠蔽』が発生し、復旧不能または無音で不正動作する。
- **F02 mock が実APIと乖離し false positive 量産**: ADR-015 A: dead_man_switch.py に OpenD + atlas-paper 死活監視を追加・... に対し『mock が実APIと乖離し false positive 量産』が発生し、復旧不能または無音で不正動作する。
- **F03 env 依存変数が未設定でテスト全スキップ**: ADR-015 A: dead_man_switch.py に OpenD + atlas-paper 死活監視を追加・... に対し『env 依存変数が未設定でテスト全スキップ』が発生し、復旧不能または無音で不正動作する。
- **F04 テストが自分のコードをimportせず常に合格**: ADR-015 A: dead_man_switch.py に OpenD + atlas-paper 死活監視を追加・... に対し『テストが自分のコードをimportせず常に合格』が発生し、復旧不能または無音で不正動作する。
- **F05 coverage 100%でも統合パスが未カバー**: ADR-015 A: dead_man_switch.py に OpenD + atlas-paper 死活監視を追加・... に対し『coverage 100%でも統合パスが未カバー』が発生し、復旧不能または無音で不正動作する。
- **F06 並列テストで共有ファイルに race condition**: ADR-015 A: dead_man_switch.py に OpenD + atlas-paper 死活監視を追加・... に対し『並列テストで共有ファイルに race condition』が発生し、復旧不能または無音で不正動作する。
- **F07 fixture のティアダウンが漏れてDB/ファイル汚染**: ADR-015 A: dead_man_switch.py に OpenD + atlas-paper 死活監視を追加・... に対し『fixture のティアダウンが漏れてDB/ファイル汚染』が発生し、復旧不能または無音で不正動作する。

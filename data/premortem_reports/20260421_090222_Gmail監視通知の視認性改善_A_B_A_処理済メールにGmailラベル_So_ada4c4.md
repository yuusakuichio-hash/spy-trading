...
- **Generated**: 2026-04-21T09:02:22
- **Source**: fallback
- **Task**: Gmail監視通知の視認性改善 A+B: (A)処理済メールにGmailラベル Sora-Lab/Important 自動付与・サブラベルMFFU/金融/SNS分類 (B)Pushover通知タイトル整形 絵文字+[タグ]+要約 例: 📧[MFFU] Tradovate認証情報発行 (C)タグトークン確認。gmail_monitor.py修正+LaunchAgent再起動。
- **Overall Risk**: medium
...
### 詳細
- **F01 依存サービス未起動でフェイル**: Gmail監視通知の視認性改善 A+B: (A)処理済メールにGmailラベル Sora-Lab/Important 自... に対し『依存サービス未起動でフェイル』が発生し、復旧不能または無音で不正動作する。
- **F02 API rate limit / auth 失敗**: Gmail監視通知の視認性改善 A+B: (A)処理済メールにGmailラベル Sora-Lab/Important 自... に対し『API rate limit / auth 失敗』が発生し、復旧不能または無音で不正動作する。
- **F03 データ型/スキーマ不整合で silent fail**: Gmail監視通知の視認性改善 A+B: (A)処理済メールにGmailラベル Sora-Lab/Important 自... に対し『データ型/スキーマ不整合で silent fail』が発生し、復旧不能または無音で不正動作する。
- **F04 タイムゾーン混在 (JST/ET/UTC)**: Gmail監視通知の視認性改善 A+B: (A)処理済メールにGmailラベル Sora-Lab/Important 自... に対し『タイムゾーン混在 (JST/ET/UTC)』が発生し、復旧不能または無音で不正動作する。
- **F05 並行実行で race condition**: Gmail監視通知の視認性改善 A+B: (A)処理済メールにGmailラベル Sora-Lab/Important 自... に対し『並行実行で race condition』が発生し、復旧不能または無音で不正動作する。
- **F06 ディスク/メモリ枯渇**: Gmail監視通知の視認性改善 A+B: (A)処理済メールにGmailラベル Sora-Lab/Important 自... に対し『ディスク/メモリ枯渇』が発生し、復旧不能または無音で不正動作する。
- **F07 冬時間/夏時間境界で off-by-one**: Gmail監視通知の視認性改善 A+B: (A)処理済メールにGmailラベル Sora-Lab/Important 自... に対し『冬時間/夏時間境界で off-by-one』が発生し、復旧不能または無音で不正動作する。
- **F08 既存ファイル破壊・未バックアップ**: Gmail監視通知の視認性改善 A+B: (A)処理済メールにGmailラベル Sora-Lab/Important 自... に対し『既存ファイル破壊・未バックアップ』が発生し、復旧不能または無音で不正動作する。
- **F09 テスト不足で本番初回に発覚**: Gmail監視通知の視認性改善 A+B: (A)処理済メールにGmailラベル Sora-Lab/Important 自... に対し『テスト不足で本番初回に発覚』が発生し、復旧不能または無音で不正動作する。
- **F10 roll-back 手順未定義で復旧不能**: Gmail監視通知の視認性改善 A+B: (A)処理済メールにGmailラベル Sora-Lab/Important 自... に対し『roll-back 手順未定義で復旧不能』が発生し、復旧不能または無音で不正動作する。

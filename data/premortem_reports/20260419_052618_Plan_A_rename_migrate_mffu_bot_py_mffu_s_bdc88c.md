...
- **Generated**: 2026-04-19T05:26:18
- **Source**: fallback
- **Task**: Plan A rename: migrate mffu_bot.py/mffu_strategy_selector.py/mffu_emergency_stop.py/mffu_rule_simulator.py to chronos_*.py. Update all imports, LaunchAgent plists, hook scripts, pushover tags to [Chronos]. Keep chronos_*.py skeleton files (already exist) by merging/absorbing content.
- **Files**: mffu_bot.py, mffu_strategy_selector.py, mffu_emergency_stop.py, mffu_rule_simulator.py, chronos_bot.py, chronos_agent.py, chronos_watchdog.py, chronos_mffu_rules.py, chronos_symbol_meta.py, chronos_pre_trade_check.py
- **Overall Risk**: medium
...
### 詳細
- **F01 依存サービス未起動でフェイル**: Plan A rename: migrate mffu_bot.py/mffu_strategy_selector.py... に対し『依存サービス未起動でフェイル』が発生し、復旧不能または無音で不正動作する。
- **F02 API rate limit / auth 失敗**: Plan A rename: migrate mffu_bot.py/mffu_strategy_selector.py... に対し『API rate limit / auth 失敗』が発生し、復旧不能または無音で不正動作する。
- **F03 データ型/スキーマ不整合で silent fail**: Plan A rename: migrate mffu_bot.py/mffu_strategy_selector.py... に対し『データ型/スキーマ不整合で silent fail』が発生し、復旧不能または無音で不正動作する。
- **F04 タイムゾーン混在 (JST/ET/UTC)**: Plan A rename: migrate mffu_bot.py/mffu_strategy_selector.py... に対し『タイムゾーン混在 (JST/ET/UTC)』が発生し、復旧不能または無音で不正動作する。
- **F05 並行実行で race condition**: Plan A rename: migrate mffu_bot.py/mffu_strategy_selector.py... に対し『並行実行で race condition』が発生し、復旧不能または無音で不正動作する。
- **F06 ディスク/メモリ枯渇**: Plan A rename: migrate mffu_bot.py/mffu_strategy_selector.py... に対し『ディスク/メモリ枯渇』が発生し、復旧不能または無音で不正動作する。
- **F07 冬時間/夏時間境界で off-by-one**: Plan A rename: migrate mffu_bot.py/mffu_strategy_selector.py... に対し『冬時間/夏時間境界で off-by-one』が発生し、復旧不能または無音で不正動作する。
- **F08 既存ファイル破壊・未バックアップ**: Plan A rename: migrate mffu_bot.py/mffu_strategy_selector.py... に対し『既存ファイル破壊・未バックアップ』が発生し、復旧不能または無音で不正動作する。
- **F09 テスト不足で本番初回に発覚**: Plan A rename: migrate mffu_bot.py/mffu_strategy_selector.py... に対し『テスト不足で本番初回に発覚』が発生し、復旧不能または無音で不正動作する。
- **F10 roll-back 手順未定義で復旧不能**: Plan A rename: migrate mffu_bot.py/mffu_strategy_selector.py... に対し『roll-back 手順未定義で復旧不能』が発生し、復旧不能または無音で不正動作する。

...
- **Source**: fallback
- **Task**: ground truth reconciliation layer market data cross-check every Bot claim validated against Yahoo SPX Tradovate market event presence within expected time window
- **Overall Risk**: medium
...
### 詳細
- **F01 flaky test がCI環境のみ失敗し本番バグを隠蔽**: ground truth reconciliation layer market data cross-check ev... に対し『flaky test がCI環境のみ失敗し本番バグを隠蔽』が発生し、復旧不能または無音で不正動作する。
- **F02 mock が実APIと乖離し false positive 量産**: ground truth reconciliation layer market data cross-check ev... に対し『mock が実APIと乖離し false positive 量産』が発生し、復旧不能または無音で不正動作する。
- **F03 env 依存変数が未設定でテスト全スキップ**: ground truth reconciliation layer market data cross-check ev... に対し『env 依存変数が未設定でテスト全スキップ』が発生し、復旧不能または無音で不正動作する。
- **F04 テストが自分のコードをimportせず常に合格**: ground truth reconciliation layer market data cross-check ev... に対し『テストが自分のコードをimportせず常に合格』が発生し、復旧不能または無音で不正動作する。
- **F05 coverage 100%でも統合パスが未カバー**: ground truth reconciliation layer market data cross-check ev... に対し『coverage 100%でも統合パスが未カバー』が発生し、復旧不能または無音で不正動作する。
- **F06 並列テストで共有ファイルに race condition**: ground truth reconciliation layer market data cross-check ev... に対し『並列テストで共有ファイルに race condition』が発生し、復旧不能または無音で不正動作する。
- **F07 fixture のティアダウンが漏れてDB/ファイル汚染**: ground truth reconciliation layer market data cross-check ev... に対し『fixture のティアダウンが漏れてDB/ファイル汚染』が発生し、復旧不能または無音で不正動作する。

...
- **Generated**: 2026-04-21T08:35:06
- **Source**: fallback
- **Task**: Atlas月利 v6実測BT: 2026-04-18以降の全施策(ORB 1DTE/delta hedge/IC sell/StraddleBuy per-symbol/center_strike per-symbol/VIX band/ATM subscribe/rate limit throttle等)を含む最新コードで ThetaData 574日(または取得済み135日+残440日DL後)フルBT実行。7戦術統合+8戦術統合で楽観/中央/保守レンジ再確定。v5推測値→v6実測値へ。月300万到達時期も再計算。
- **Overall Risk**: medium
...
### 詳細
- **F01 依存サービス未起動でフェイル**: Atlas月利 v6実測BT: 2026-04-18以降の全施策(ORB 1DTE/delta hedge/IC sel... に対し『依存サービス未起動でフェイル』が発生し、復旧不能または無音で不正動作する。
- **F02 API rate limit / auth 失敗**: Atlas月利 v6実測BT: 2026-04-18以降の全施策(ORB 1DTE/delta hedge/IC sel... に対し『API rate limit / auth 失敗』が発生し、復旧不能または無音で不正動作する。
- **F03 データ型/スキーマ不整合で silent fail**: Atlas月利 v6実測BT: 2026-04-18以降の全施策(ORB 1DTE/delta hedge/IC sel... に対し『データ型/スキーマ不整合で silent fail』が発生し、復旧不能または無音で不正動作する。
- **F04 タイムゾーン混在 (JST/ET/UTC)**: Atlas月利 v6実測BT: 2026-04-18以降の全施策(ORB 1DTE/delta hedge/IC sel... に対し『タイムゾーン混在 (JST/ET/UTC)』が発生し、復旧不能または無音で不正動作する。
- **F05 並行実行で race condition**: Atlas月利 v6実測BT: 2026-04-18以降の全施策(ORB 1DTE/delta hedge/IC sel... に対し『並行実行で race condition』が発生し、復旧不能または無音で不正動作する。
- **F06 ディスク/メモリ枯渇**: Atlas月利 v6実測BT: 2026-04-18以降の全施策(ORB 1DTE/delta hedge/IC sel... に対し『ディスク/メモリ枯渇』が発生し、復旧不能または無音で不正動作する。
- **F07 冬時間/夏時間境界で off-by-one**: Atlas月利 v6実測BT: 2026-04-18以降の全施策(ORB 1DTE/delta hedge/IC sel... に対し『冬時間/夏時間境界で off-by-one』が発生し、復旧不能または無音で不正動作する。
- **F08 既存ファイル破壊・未バックアップ**: Atlas月利 v6実測BT: 2026-04-18以降の全施策(ORB 1DTE/delta hedge/IC sel... に対し『既存ファイル破壊・未バックアップ』が発生し、復旧不能または無音で不正動作する。
- **F09 テスト不足で本番初回に発覚**: Atlas月利 v6実測BT: 2026-04-18以降の全施策(ORB 1DTE/delta hedge/IC sel... に対し『テスト不足で本番初回に発覚』が発生し、復旧不能または無音で不正動作する。
- **F10 roll-back 手順未定義で復旧不能**: Atlas月利 v6実測BT: 2026-04-18以降の全施策(ORB 1DTE/delta hedge/IC sel... に対し『roll-back 手順未定義で復旧不能』が発生し、復旧不能または無音で不正動作する。

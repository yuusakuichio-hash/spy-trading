...
- **Generated**: 2026-04-20T19:34:11
- **Source**: fallback
- **Task**: cycle8 MUST-FIX β 8件+30直接テスト: T1 News TZ ET化・_is_order_sent組込・peak_balanceアカ別・last_trade_date・CI mutmut pin・plan_id fail-closed・core_50k除去・直接テスト30本
- **Overall Risk**: medium
...
### 詳細
- **F01 依存サービス未起動でフェイル**: cycle8 MUST-FIX β 8件+30直接テスト: T1 News TZ ET化・_is_order_sent組... に対し『依存サービス未起動でフェイル』が発生し、復旧不能または無音で不正動作する。
- **F02 API rate limit / auth 失敗**: cycle8 MUST-FIX β 8件+30直接テスト: T1 News TZ ET化・_is_order_sent組... に対し『API rate limit / auth 失敗』が発生し、復旧不能または無音で不正動作する。
- **F03 データ型/スキーマ不整合で silent fail**: cycle8 MUST-FIX β 8件+30直接テスト: T1 News TZ ET化・_is_order_sent組... に対し『データ型/スキーマ不整合で silent fail』が発生し、復旧不能または無音で不正動作する。
- **F04 タイムゾーン混在 (JST/ET/UTC)**: cycle8 MUST-FIX β 8件+30直接テスト: T1 News TZ ET化・_is_order_sent組... に対し『タイムゾーン混在 (JST/ET/UTC)』が発生し、復旧不能または無音で不正動作する。
- **F05 並行実行で race condition**: cycle8 MUST-FIX β 8件+30直接テスト: T1 News TZ ET化・_is_order_sent組... に対し『並行実行で race condition』が発生し、復旧不能または無音で不正動作する。
- **F06 ディスク/メモリ枯渇**: cycle8 MUST-FIX β 8件+30直接テスト: T1 News TZ ET化・_is_order_sent組... に対し『ディスク/メモリ枯渇』が発生し、復旧不能または無音で不正動作する。
- **F07 冬時間/夏時間境界で off-by-one**: cycle8 MUST-FIX β 8件+30直接テスト: T1 News TZ ET化・_is_order_sent組... に対し『冬時間/夏時間境界で off-by-one』が発生し、復旧不能または無音で不正動作する。
- **F08 既存ファイル破壊・未バックアップ**: cycle8 MUST-FIX β 8件+30直接テスト: T1 News TZ ET化・_is_order_sent組... に対し『既存ファイル破壊・未バックアップ』が発生し、復旧不能または無音で不正動作する。
- **F09 テスト不足で本番初回に発覚**: cycle8 MUST-FIX β 8件+30直接テスト: T1 News TZ ET化・_is_order_sent組... に対し『テスト不足で本番初回に発覚』が発生し、復旧不能または無音で不正動作する。
- **F10 roll-back 手順未定義で復旧不能**: cycle8 MUST-FIX β 8件+30直接テスト: T1 News TZ ET化・_is_order_sent組... に対し『roll-back 手順未定義で復旧不能』が発生し、復旧不能または無音で不正動作する。

...
- **Generated**: 2026-04-21T09:40:34
- **Source**: fallback
- **Task**: ORB 1DTE QQQ/IWM データ取得+BT完了 (ThetaTerminal再起動+download_1dte_data実行+全データDL+backtest_orb_1dte再実行+QQQ/IWM PASS判定+strategy_selector組込) / Gamma Scalping 本格MVP実装(既存GammaScalpEngine拡張ではなくengines/gamma_scalp.py新規作成・delta中立維持・volatility gain収益化・ペーパーsmoke・BT近似・2-3h完遂目標)。工数分単位・先延ばし禁止・見積もり2-3h目安。
- **Overall Risk**: medium
...
### 詳細
- **F01 依存サービス未起動でフェイル**: ORB 1DTE QQQ/IWM データ取得+BT完了 (ThetaTerminal再起動+download_1dte_... に対し『依存サービス未起動でフェイル』が発生し、復旧不能または無音で不正動作する。
- **F02 API rate limit / auth 失敗**: ORB 1DTE QQQ/IWM データ取得+BT完了 (ThetaTerminal再起動+download_1dte_... に対し『API rate limit / auth 失敗』が発生し、復旧不能または無音で不正動作する。
- **F03 データ型/スキーマ不整合で silent fail**: ORB 1DTE QQQ/IWM データ取得+BT完了 (ThetaTerminal再起動+download_1dte_... に対し『データ型/スキーマ不整合で silent fail』が発生し、復旧不能または無音で不正動作する。
- **F04 タイムゾーン混在 (JST/ET/UTC)**: ORB 1DTE QQQ/IWM データ取得+BT完了 (ThetaTerminal再起動+download_1dte_... に対し『タイムゾーン混在 (JST/ET/UTC)』が発生し、復旧不能または無音で不正動作する。
- **F05 並行実行で race condition**: ORB 1DTE QQQ/IWM データ取得+BT完了 (ThetaTerminal再起動+download_1dte_... に対し『並行実行で race condition』が発生し、復旧不能または無音で不正動作する。
- **F06 ディスク/メモリ枯渇**: ORB 1DTE QQQ/IWM データ取得+BT完了 (ThetaTerminal再起動+download_1dte_... に対し『ディスク/メモリ枯渇』が発生し、復旧不能または無音で不正動作する。
- **F07 冬時間/夏時間境界で off-by-one**: ORB 1DTE QQQ/IWM データ取得+BT完了 (ThetaTerminal再起動+download_1dte_... に対し『冬時間/夏時間境界で off-by-one』が発生し、復旧不能または無音で不正動作する。
- **F08 既存ファイル破壊・未バックアップ**: ORB 1DTE QQQ/IWM データ取得+BT完了 (ThetaTerminal再起動+download_1dte_... に対し『既存ファイル破壊・未バックアップ』が発生し、復旧不能または無音で不正動作する。
- **F09 テスト不足で本番初回に発覚**: ORB 1DTE QQQ/IWM データ取得+BT完了 (ThetaTerminal再起動+download_1dte_... に対し『テスト不足で本番初回に発覚』が発生し、復旧不能または無音で不正動作する。
- **F10 roll-back 手順未定義で復旧不能**: ORB 1DTE QQQ/IWM データ取得+BT完了 (ThetaTerminal再起動+download_1dte_... に対し『roll-back 手順未定義で復旧不能』が発生し、復旧不能または無音で不正動作する。

...
- **Generated**: 2026-04-21T05:01:50
- **Source**: fallback
- **Task**: gmail_monitor.pyをローカルMac向けに修正(VPSパス→ローカルパス)+初回OAuth認証+LaunchAgent登録(15分ポーリング)+MFFU即検知ルール追加。credentials.json配置済・chmod600・gitignore済。
- **Overall Risk**: medium
...
### 詳細
- **F01 依存サービス未起動でフェイル**: gmail_monitor.pyをローカルMac向けに修正(VPSパス→ローカルパス)+初回OAuth認証+Launch... に対し『依存サービス未起動でフェイル』が発生し、復旧不能または無音で不正動作する。
- **F02 API rate limit / auth 失敗**: gmail_monitor.pyをローカルMac向けに修正(VPSパス→ローカルパス)+初回OAuth認証+Launch... に対し『API rate limit / auth 失敗』が発生し、復旧不能または無音で不正動作する。
- **F03 データ型/スキーマ不整合で silent fail**: gmail_monitor.pyをローカルMac向けに修正(VPSパス→ローカルパス)+初回OAuth認証+Launch... に対し『データ型/スキーマ不整合で silent fail』が発生し、復旧不能または無音で不正動作する。
- **F04 タイムゾーン混在 (JST/ET/UTC)**: gmail_monitor.pyをローカルMac向けに修正(VPSパス→ローカルパス)+初回OAuth認証+Launch... に対し『タイムゾーン混在 (JST/ET/UTC)』が発生し、復旧不能または無音で不正動作する。
- **F05 並行実行で race condition**: gmail_monitor.pyをローカルMac向けに修正(VPSパス→ローカルパス)+初回OAuth認証+Launch... に対し『並行実行で race condition』が発生し、復旧不能または無音で不正動作する。
- **F06 ディスク/メモリ枯渇**: gmail_monitor.pyをローカルMac向けに修正(VPSパス→ローカルパス)+初回OAuth認証+Launch... に対し『ディスク/メモリ枯渇』が発生し、復旧不能または無音で不正動作する。
- **F07 冬時間/夏時間境界で off-by-one**: gmail_monitor.pyをローカルMac向けに修正(VPSパス→ローカルパス)+初回OAuth認証+Launch... に対し『冬時間/夏時間境界で off-by-one』が発生し、復旧不能または無音で不正動作する。
- **F08 既存ファイル破壊・未バックアップ**: gmail_monitor.pyをローカルMac向けに修正(VPSパス→ローカルパス)+初回OAuth認証+Launch... に対し『既存ファイル破壊・未バックアップ』が発生し、復旧不能または無音で不正動作する。
- **F09 テスト不足で本番初回に発覚**: gmail_monitor.pyをローカルMac向けに修正(VPSパス→ローカルパス)+初回OAuth認証+Launch... に対し『テスト不足で本番初回に発覚』が発生し、復旧不能または無音で不正動作する。
- **F10 roll-back 手順未定義で復旧不能**: gmail_monitor.pyをローカルMac向けに修正(VPSパス→ローカルパス)+初回OAuth認証+Launch... に対し『roll-back 手順未定義で復旧不能』が発生し、復旧不能または無音で不正動作する。

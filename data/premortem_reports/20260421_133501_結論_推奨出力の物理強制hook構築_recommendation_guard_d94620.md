...
- **Generated**: 2026-04-21T13:35:01
- **Source**: fallback
- **Task**: 結論/推奨出力の物理強制hook構築: recommendation_guard.sh 新規作成。「推奨/最適/べき/不可/確定/破綻」等の結論語句検出時に、直近30分以内の事前調査ファイル(data/research/*.md)または bottleneck分析の存在を確認。なければexit 2 HARD BLOCK。ボトルネック洗出し前の推奨を物理的に防ぐ。secretary/builder全agentに適用。
- **Overall Risk**: medium
...
### 詳細
- **F01 依存サービス未起動でフェイル**: 結論/推奨出力の物理強制hook構築: recommendation_guard.sh 新規作成。「推奨/最適/べき/不... に対し『依存サービス未起動でフェイル』が発生し、復旧不能または無音で不正動作する。
- **F02 API rate limit / auth 失敗**: 結論/推奨出力の物理強制hook構築: recommendation_guard.sh 新規作成。「推奨/最適/べき/不... に対し『API rate limit / auth 失敗』が発生し、復旧不能または無音で不正動作する。
- **F03 データ型/スキーマ不整合で silent fail**: 結論/推奨出力の物理強制hook構築: recommendation_guard.sh 新規作成。「推奨/最適/べき/不... に対し『データ型/スキーマ不整合で silent fail』が発生し、復旧不能または無音で不正動作する。
- **F04 タイムゾーン混在 (JST/ET/UTC)**: 結論/推奨出力の物理強制hook構築: recommendation_guard.sh 新規作成。「推奨/最適/べき/不... に対し『タイムゾーン混在 (JST/ET/UTC)』が発生し、復旧不能または無音で不正動作する。
- **F05 並行実行で race condition**: 結論/推奨出力の物理強制hook構築: recommendation_guard.sh 新規作成。「推奨/最適/べき/不... に対し『並行実行で race condition』が発生し、復旧不能または無音で不正動作する。
- **F06 ディスク/メモリ枯渇**: 結論/推奨出力の物理強制hook構築: recommendation_guard.sh 新規作成。「推奨/最適/べき/不... に対し『ディスク/メモリ枯渇』が発生し、復旧不能または無音で不正動作する。
- **F07 冬時間/夏時間境界で off-by-one**: 結論/推奨出力の物理強制hook構築: recommendation_guard.sh 新規作成。「推奨/最適/べき/不... に対し『冬時間/夏時間境界で off-by-one』が発生し、復旧不能または無音で不正動作する。
- **F08 既存ファイル破壊・未バックアップ**: 結論/推奨出力の物理強制hook構築: recommendation_guard.sh 新規作成。「推奨/最適/べき/不... に対し『既存ファイル破壊・未バックアップ』が発生し、復旧不能または無音で不正動作する。
- **F09 テスト不足で本番初回に発覚**: 結論/推奨出力の物理強制hook構築: recommendation_guard.sh 新規作成。「推奨/最適/べき/不... に対し『テスト不足で本番初回に発覚』が発生し、復旧不能または無音で不正動作する。
- **F10 roll-back 手順未定義で復旧不能**: 結論/推奨出力の物理強制hook構築: recommendation_guard.sh 新規作成。「推奨/最適/べき/不... に対し『roll-back 手順未定義で復旧不能』が発生し、復旧不能または無音で不正動作する。

...
- **Generated**: 2026-04-19T05:49:34
- **Source**: fallback
- **Task**: Implement Max Loss 100usd survival mode for Chronos Sim-Funded after first payout. Elite MES trader logic: max 1 contract, daily loss stop 80usd, per-trade stop 30usd, profit lock 150usd then stop for day, max 1 trade per day, preferred A-plus setup windows only (10:30-11:00 ET main), transition detection post first payout
...
### 詳細
- **F01 依存サービス未起動でフェイル**: Implement Max Loss 100usd survival mode for Chronos Sim-Fund... に対し『依存サービス未起動でフェイル』が発生し、復旧不能または無音で不正動作する。
- **F02 API rate limit / auth 失敗**: Implement Max Loss 100usd survival mode for Chronos Sim-Fund... に対し『API rate limit / auth 失敗』が発生し、復旧不能または無音で不正動作する。
- **F03 データ型/スキーマ不整合で silent fail**: Implement Max Loss 100usd survival mode for Chronos Sim-Fund... に対し『データ型/スキーマ不整合で silent fail』が発生し、復旧不能または無音で不正動作する。
- **F04 タイムゾーン混在 (JST/ET/UTC)**: Implement Max Loss 100usd survival mode for Chronos Sim-Fund... に対し『タイムゾーン混在 (JST/ET/UTC)』が発生し、復旧不能または無音で不正動作する。
- **F05 並行実行で race condition**: Implement Max Loss 100usd survival mode for Chronos Sim-Fund... に対し『並行実行で race condition』が発生し、復旧不能または無音で不正動作する。
- **F06 ディスク/メモリ枯渇**: Implement Max Loss 100usd survival mode for Chronos Sim-Fund... に対し『ディスク/メモリ枯渇』が発生し、復旧不能または無音で不正動作する。
- **F07 冬時間/夏時間境界で off-by-one**: Implement Max Loss 100usd survival mode for Chronos Sim-Fund... に対し『冬時間/夏時間境界で off-by-one』が発生し、復旧不能または無音で不正動作する。
- **F08 既存ファイル破壊・未バックアップ**: Implement Max Loss 100usd survival mode for Chronos Sim-Fund... に対し『既存ファイル破壊・未バックアップ』が発生し、復旧不能または無音で不正動作する。
- **F09 テスト不足で本番初回に発覚**: Implement Max Loss 100usd survival mode for Chronos Sim-Fund... に対し『テスト不足で本番初回に発覚』が発生し、復旧不能または無音で不正動作する。
- **F10 roll-back 手順未定義で復旧不能**: Implement Max Loss 100usd survival mode for Chronos Sim-Fund... に対し『roll-back 手順未定義で復旧不能』が発生し、復旧不能または無音で不正動作する。

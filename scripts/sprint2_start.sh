...
# Step 0: ADR-015 判断確認（Strategist 指摘でゆうさくさん承認必要）
echo ""
echo "─── Step 0: ADR-015 判断確認 ───"
echo "  Strategist r8 T-1 指摘: ADR-014 Decision 1/2 は越権の可能性"
echo "  代替案: data/decisions/ADR-015-moomoo-provider-alternatives-20260424.md"
echo ""
echo "  A) Mac mini + caffeinate + Sentinel 死活監視"
echo "  B) yfinance auto-fallback default + 遅延 Pushover"
echo "  C) ADR-014 そのまま (Secretary 判断尊重)"
echo "  D) 再設計"
if [ "$DRY_RUN" -eq 0 ]; then
    if ! confirm "ADR-015 を読みましたか？（A/B/C/D 選択は Sprint 2 本着手時に実施）"; then
        echo "  ⚠ ADR-015 確認してから再実行推奨"
        exit 1
    fi
fi

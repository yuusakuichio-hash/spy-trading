...
def run_aar(target_date: date | None = None) -> Path:
    ...
    try:
        ...
        from scripts.trader_evaluation import run_evaluation, _fmt
        eval_result = run_evaluation(period="daily", target_date=target_date)
        if "error" not in eval_result:
            m = eval_result.get("metrics", {})
            pf = m.get("M1_profit_factor")
            wr = m.get("M2_win_rate")
            disc = m.get("M13_discipline_score")
            sortino = m.get("M6_sortino")
            pcr = eval_result.get("premium_capture_rate_pct")
            ev1 = eval_result.get("ev_gaps", {}).get("EV1_hypothesis_match", {})
            lines_out += [
                f"## f) 優秀トレーダー判定（M1-M15）",
                f"",
                f"| 指標 | 値 | 判定 |",
                f"|------|-----|------|",
                f"| M1 Profit Factor | {_fmt(pf)} | {'GOOD' if pf and (pf == float('inf') or pf >= 1.5) else 'POOR'} |",
                f"| M2 勝率 | {_fmt(wr, '%', 1, pct_scale=True)} | {'GOOD' if wr and wr >= 0.66 else 'WARN'} |",
                f"| M13 Discipline Score | {_fmt(disc, '%', 1, pct_scale=True)} | {'GOOD' if disc and disc >= 0.95 else ('WARN' if disc and disc >= 0.90 else 'POOR')} |",
                f"| M6 Sortino Ratio | {_fmt(sortino)} | {'GOOD' if sortino and sortino >= 2.0 else 'N/A'} |",
                f"| Premium Capture Rate | {_fmt(pcr, '%', 2)} | {'GOOD' if pcr and pcr >= 5.65 else ('WARN' if pcr and pcr >= 3.5 else 'N/A')} |",
                f"| EV-1 仮説マッチ率 | {_fmt(ev1.get('match_rate'), '%', 1, pct_scale=True)} | {'GOOD' if ev1.get('match_rate') and ev1['match_rate'] >= 0.7 else 'WARN'} |",
                f"",
                f"詳細レポート: data/eval/trader_eval_{target_date.isoformat().replace('-', '')}.md",
                f"",
            ]
        else:
            lines_out += [
                f"## f) 優秀トレーダー判定（M1-M15）",
                f"",
                f"評価データなし: {eval_result.get('error', 'unknown error')}",
                f"",
            ]
    ...
    try:
        import sys as _sys
        _sys.path.insert(0, str(BASE))
        from common.pdt_tracker import get_global_tracker as _get_pdt_tracker
        _pdt = _get_pdt_tracker()
        # capital_usd は planned から取得、なければデフォルト値
        _capital = float(planned.get("capital_usd", planned.get("capital", 8000.0)) or 8000.0)
        _pdt_status = _pdt.get_status(_capital)
        _remaining = _pdt_status.get("pdt_remaining", "N/A")
        _constrained = _pdt_status.get("pdt_constrained", True)
        _rolling5 = _pdt_status.get("rolling5_count", 0)
        _biz_window = _pdt_status.get("business_days_window", [])
        _biz_str = ", ".join(_biz_window) if _biz_window else "N/A"
        lines_out += [
            f"## g) PDT状況（第7章）",
            f"",
            f"| 項目 | 値 |",
            f"|---|---|",
            f"| PDT制限中 | {'YES ($25K未満)' if _constrained else 'NO ($25K以上)'} |",
            f"| PDT残本数 | {_remaining} |",
            f"| 直近5営業日 day_trade数 | {_rolling5} / {'3' if _constrained else '無制限'} |",
            f"| 5営業日ウィンドウ | {_biz_str} |",
            f"| can_enter | {'YES' if _pdt_status.get('can_enter', True) else 'NO (ブロック中)'} |",
            f"",
        ]
    ...
    try:
        ...
        from calc_discount_factor import load_records as _load_df_records, calc_stats as _calc_df_stats
        _df_rows = _load_df_records()
        _df_stats = _calc_df_stats(_df_rows)
        _df_total = _df_stats["total_count"]
        _df_ov = _df_stats.get("overall") or {}
        _df_median = _df_ov.get("median", "N/A")
        _df_mean   = _df_ov.get("mean",   "N/A")
        if isinstance(_df_median, float) and _df_total >= 10:
            _df_corrected = round(8 * _df_median / 0.50, 1)
            _df_corrected_str = f"{_df_corrected}%"
        else:
            _df_corrected_str = "N/A (10件未満)"
        _df_by_strat = _df_stats.get("by_strategy", {})
        _df_strat_rows = "\n".join(
            f"| {k} | {v.get('count', 0)} | {v.get('median', 'N/A')} | {v.get('mean', 'N/A')} |"
            for k, v in sorted(_df_by_strat.items())
        ) or "| (記録なし) | - | - | - |"
        lines_out += [
            f"## h) 割引率フィードバック（第8章）",
            f"",
            ...
            f"|---|---|",
            f"| サンプル数 | {_df_total}件 |",
            f"| 実測割引率（中央値） | {_df_median} |",
            f"| 実測割引率（平均） | {_df_mean} |",
            f"| 補正後中央月利（BT 8%ベース） | {_df_corrected_str} |",
            f"",
            f"### 戦術別割引率",
            f"",
            f"| 戦術 | 件数 | 中央値 | 平均 |",

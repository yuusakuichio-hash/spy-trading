...
7. **ジャーナル領域隔離**（2026-04-23 ゆうさくさん承認）— 本音・感情・身体・家族・音楽個人事情は `~/sora_journal/` で扱う。このディレクトリの jsonl/memory/data から personal 情報を検索・プロジェクト判断に流用禁止
...
- **2027-04**: 月 300 万達成（4 層構造: Bot 複利 + SNS + 実績証明 + 私募ファンド）
  - ※ C2（Collective2）は 2026-04-16 に法的リスクで除外済（`memory/project_c2_excluded.md`）
...
## ジャーナル連携トリガー（2026-04-23 追加）

- **トリガー語**: 「ジャーナルに連携」「これジャーナル側にだけ」等
- **検知したら** `~/sora_journal/spec/journal_sync_trigger.md` のフィルタ仕様に従う
- **出力先**: `~/sora_journal/inbox/YYYYMMDD_HHMM_<topic>.md`
- **完了報告は 1 行のみ・中身再掲禁止・プロジェクト作業に戻る**
